...
```
[Photo: Scanned document with no visible text or diagrams]
```
...
I'm unable to transcribe any text or provide Markdown formatting for the content as the page appears to be blank.
...
The manual is completely reorganized and most parts are expanded or rewritten. More examples are included and the manual now covers all programming libraries of data communications available through COSMOS:
...
The image appears to be blank, and no text or diagrams are visible to transcribe or convert to Markdown.
...
| 6.8.1 | TLIB Size | 211 |
| 6.8.2 | Speed | 211 |
| 7 | TLIB/PLANC REFERENCE GUIDE | 213 |
...
| 8 | TLIB/FORTRAN REFERENCE GUIDE | 235 |
...
| Function                                | Page |
|-----------------------------------------|------|
| Open a Port (XFGOPN)                    | 262  |
| Close Ports (XFCLS)                     | 262  |
| Port Status (XFPST)                     | 262  |
| General Status (XFGST)                  | 264  |
| Disconnect (XFDCT)                      | 265  |
...
| Function                                                | Page |
|---------------------------------------------------------|------|
| Reserving Message Buffer (XFGET)                        | 266  |
| Defining a User Buffer (XPUBB)                          | 267  |
| Releasing Message Buffer (XFREL)                        | 267  |
| Allocating Message Buffers (XFALM)                      | 268  |
| Freeing Allocated Message Buffers (XFFRM)               | 269  |
| Writing into Message Buffers (XFWR)                     | 269  |
| Writing only the Header of a Message Buffer (XFWHd)     | 270  |
| Reading from a Message Buffer (XFREA)                   | 270  |
| Reading only the Header of a Message Buffer (XFRHD)     | 271  |
| Define Bank Number for Drivers (XFDBK)                  | 271  |
| Sending Message (XFSND)                                 | 272  |
| Returning a Message (XFRTN)                             | 274  |
| Receiving Next Message (XFRCV)                          | 275  |
| Receive and Read Header (XFRRH)                         | 276  |
| Receive and Read Message (XFRRE)                        | 277  |
| List Messages and Ports (XFLMP)                         | 277  |
| Message Status (XFMST)                                  | 278  |
| Set Current Message (XFSCM)                             | 278  |
...
| Function                                        | Page |
|-------------------------------------------------|------|
| Dummy function (XFDUM)                          | 279  |
| Start Multi-Call (XFSMC)                        | 280  |
| Define Maximum Memory (XFDMM)                   | 280  |
| Convert Magic Number to Port and System Number  | 281  |
| Convert Port Number to Magic Number (XFP2M)     | 281  |
| Define Wake Up Context (XFWDF)                  | 282  |
| Check System and User Privileges (XFCPV)        | 282  |
...
| 3.5 | Send Letter and Kick (XSLKEK) | 292 |
...
| 3.21 | Set Crash Information (XSSCI) | 300 |
| 3.22 | Get/Check Attribute (XSGAT) | 302 |
| 3.23 | Define/Remove Attribute (XSDAT) | 304 |
...
| Section | Page |
|---------|------|
| C | The ND-100 XMSG System From PIOC | 307 |
...
Messages are by default nonsecure. Nonsecure messages are discarded by XMSG if they cannot be delivered, or are queued on a port that gets closed, and their message buffers are released.
...
```
returnStatus = xmpfrel(0,msgIdentifier)
```

Normal return status is zero. If you include the appropriate `:DEFS` file in your source code, you may use the symbol `XMOK` for the zero status. The file is called `XMP:DEFS` for PLANC and `XMF:DEFS` for FORTRAN. If a call is not terminated, the return status is `MXNENTM` (e.g., if no message is waiting when `XMPFRCV` is called without the `XFWTF` option, `MXNENTM`). Other return statuses are error codes. A list of the error codes plus their corresponding symbols is provided in appendix D. However, note that the routines used for buffer formatting will, if an error is encountered, return `-1` as the error code.
...
For the use of one single option in a call, please see the XMPFGST or the XMFFGST routine. For using several options simultaneously in one call we refer you to XMPFSND or XMTFSND.
...
| XMPF2PM      | Convert port number to magic number          | XMSG     |
...
| 2   | offSet<br>Bytes       | RW  | Current number of bytes used in outBuffer.|
| 3   | paramValue<br>Integer | R   | 32-bit value to be coded.                 |
| 4   | paramNumber<br>Integer| R   | Parameter number.                         |
...
| No. | Parameter Name/Type | R/W | Explanation                        |
|-----|---------------------|-----|------------------------------------|
| 1   | outBuffer<br/>Bytes | R   | Local user buffer.                 |
| 2   | offSet<br/>Integer  | RW  | Current number of bytes used Buffer. |
| 3   | paramValue<br/>Integer2 | R   | 16-bit value to be coded.          |
| 4   | paramNumber<br/>Integer | R   | Parameter number.                  |
...
% Check returnStatus, and if OK, put in
% the serial number and the service number
xmpbrdy(myBuff,XSGIN,serialnumber) =: returnStatus
```
...
Note that this routine just prepares the letter in a local buffer (`headerBuffer`). It does not copy the header into an XMSG message buffer, nor does it send anything. The data copying and sending must be done using routines such as XMPFWR1 and XMPROUT.
...
Scanned by Jonny Oddberg for Sintran Data © 2021.
...
lll =: serialNumber
% Give xmplet more than enough space for formatting.
60 =: lengthBuffer
% Let xmplet create the necessary header.
xmplet(myBuffer,lengthBuffer,offset,serialNumber,&
...
xmplet(headerBuffer,lengthBuffer,offset,&
...
| No: | Parameter Name/ Type: | R/W | Explanation: |
|-----|-----------------------|-----|--------------|
| 1   | localBuffer<br>Bytes  | R   | Local user buffer. |
| 2   | paramNumber<br>Integer | R   | Parameter number of the parameter that is to be found. |
| 3   | startOfParam<br>Integer | W  | Displacement within localBuffer in bytes. |
| 4   | paramType<br>Bytes    | W   | Indicates the parameter type of paramNumber. The parameter type is returned as INTEGER or STRING. |
| 5   | paramLength<br>Integer | W  | Length of parameter in bytes. |
...
The routine will search through localBuffer for the parameter number specified by paramNumber. If the parameter is found, paramType indicates the parameter type of the located parameter, startOfParam indicates the position of the first significant byte in the parameter, and paramLength gives the number of significant bytes in the parameter. If no parameter with the requested parameter number is found, -1 is returned as error code in returnStatus.

Note that localBuffer must start on an even byte boundary. If not, -1 will be returned as error code in returnStatus.

### RULES
The user buffer must have been coded according to the XROUT message format. Permitted for both non-privileged and privileged tasks.
...
```
% Specify and locate parameter number 1.
l =: paramNumber
xmploc(localBuffer,paramNumber,startOfParam,&
    paramType,paramLength) =: returnStatus
...
The example builds a buffer for the 'Send Letter' (XSLET) service request. It is similar to the behavior of routine XMPLET.
...
% the serial number and the service number XSLET
...
```plaintext
xmpconf(0, XMSGpassword, configMask, XMSGrestartCnt) =: returnStatus
```
...
| --- | -------------------- | --- | ------------ |
| 1   | flags Integer        | R   | Options. |
| 2   | userBuffer Xmuseraddress Integer | R | Address of the user buffer. |
...
**EXAMPLE**: xmpfabr(0, userBuffer, userDisp, readLength, memoryDisp, bankNumber) =: returnStatus
...
|-----|---------------------|-----|-------------|
| 1   | flags Integer       | R   | Options.    |
| 2   | msgIdentifier Xmmsgidentifier Integer | R | The identifier of a received message. |
...
| 3   | registerBlock XmuserAddress | R   | Address of register block (an 8-word buffer).                 |
...
`XFPPON` - Paging on. This must be set if the driver is running with paging on.
...
```
2**XFPPON =: flags
xmpfcrd(flags, interruptLevel, registerBlock, &
        XTblockAddress) =: returnStatus;
```
...
- **User disconnect:**
  ...
  - On logout or RT program termination
...
|-----|----------------------|-----|----------------------------------------|
| 1   | flags Integer        | R   | Options.                               |
| 2   | bufferAddress Integer4 | R | Address of physical memory buffer.     |
...
xmpfdub(0,bufferAddress,bufferSize) =: returnStatus
...
### OPTIONS
- **XFWTFF** - Wait flag. If no message buffer of the requested size is available, the task will be suspended. Execution resumes when a buffer becomes available.
...
| 1   | flags               | R   | Options.                          |
|     | Integer             |     |                                   |
| 2   | startScanPort       | R   | Last port to be scanned.          |
|     | Integer             |     |                                   |
| 3   | portNumber          | W   | Port number where the message is  |
|     | Integer             |     | waiting.                          |
...
The parameter `startScanPort` specifies the last port to be searched. If `startScanPort` is zero, this implies the most recently opened port (i.e., the default port). Note that the search will begin with the next port (if any) after that specified, and then follow the task's port list (see example below).
...
is waiting on any of the ports, XMSG will clear both the ‘general wake up’ bit for the task and the ‘wake up’ bit on all ports opened by the task before checking the requested option(s). (The ‘general wake up’ bit may have been set as a result from a previously executed XMPFGST call, and the ‘wake up’ bit on the individual ports may have been set as a result from previously executed XMPFPST, XMPFPRCV, XMPFPRRH or XMPFPRRE calls.)
...
|-----|---------------------------|-----|---------------------------------------------------------------|
| 1   | flags                     | R   | Options.                                                      |
| 2   | msgIdentifier XmmsgIdentifier | R   | Message identifier or 0.                                      |
| 3   | portNo Integer            | R   | Port number or 0.                                             |
| 4   | msgIdentFound XmmsgIdentifier | W   | Message ID for the first message found equal to or greater than requested. |
| 5   | msgSize Integer           | W   | Message size of msgIdentFound in bytes.                       |
| 6   | portNoFound Integer       | W   | Port no. of the first port found equal to or greater than requested. |
...
### Example
```plaintext
xmpflmp(0,msgIdentifier,portNo,msgIdentFound,
       & msgSize,portNoFound) =: returnStatus
```
...
|    | Integer             |     |                                        |
+----+---------------------+-----+----------------------------------------+
| 2  | magicNumber         | R   | Magic number.                          |
|    | Integer4            |     |                                        |
+----+---------------------+-----+----------------------------------------+
| 3  | portNumber          | W   | Port number.                           |
|    | Integer             |     |                                        |
+----+---------------------+-----+----------------------------------------+
| 4  | systemNumber        | W   | System number.                         |
|    | Integer             |     |                                        |
+----+---------------------+-----+----------------------------------------+
| 5  | RTinOrOtherInfo     | W   | RT index or other information. See     |
|    | Integer             |     | below.                                 |
+----+---------------------+-----+----------------------------------------+
| 6  | additionalInfo      | W   | Additional information. See below.     |
|    | Integer             |     |                                        |
+----+---------------------+-----+----------------------------------------+
```
...
```plaintext
xmpfm2p(0,magicNumber,portNumber,systemNumber,&
RTinOrOtherInfo,additionalInfo) ::= returnStatus
```
...
|-----|---------------------------|-----|---------------------------------------|
| 1   | flags Integer             | R   | Options.                              |
| 2   | msgIdentifier Xmmsgidentifier Integer | R | Message identifier.                  |
| 3   | msgType Integer           | W   | Message type, see explanation below.  |
| 4   | remoteMagicNum Integer4   | W   | Magic number of sending port.         |
...
### EXAMPLE
```plaintext
xmpfmst(0,msgIdentifier,msgType,remoteMagicNum,&
msgLength) =: returnStatus
```
...
The caller must be either a driver, a direct task, a foreground program, or a background program logged in as a user system. Besides this, the program must also specify the current XMSG password, which can be obtained using the XMPCONF routine.
...
| 5   | msgIdentifier Integer | W   | Message identifier.                       |
...
On return from the routine, `msgType` indicates the message type of the first message queued to `localPort`.  
If no message is waiting, `msgType` is zero. If a message is waiting, the `remotePort`, `msgIdentifier`, and `queueLength` parameters will contain the hashed magic number of the sending port, the message address, and the number of messages chained to `localPort`.

If no message is waiting on `localPort` and the `XFMTF` flag is not set, the base value of the error codes (`XMXENTM`) is returned as `returnStatus`.
...
- **XMTNO** - Normal message
- **XMROU** - Message last sent by XMROUT (routing program)
- **XMTHI** - High priority message (sent with XFHIP option)
- **XMTRE** - Returned message (sent secure but could not be delivered)
...
| 3   | msgType Integer                      | W   | Message type, see explanation below.                           |
| 4   | remotePort Integer                   | W   | Hashed magic number of remote port.                            |
| 5   | msgIdentifier Xmnsgidentifier        | W   | Message identifier.                                            |
...
If no message is waiting on localPort and the XFTWT flag is not set, the base value of the error codes {XMXENTM} is returned as returnStatus.
...
Note that calling this routine when a message is waiting on localPort will lead to the clearing of both the 'general wake up' bit for the task and the 'wake up' bit on localPort. If no message is waiting on localPort, XMSG will clear both the 'general wake up' bit for the task and the 'wake up' bit on localPort before checking the requested option(s). (The 'general wake up' bit may have been set as a result from a previously executed XMPFGST call, and the 'wake up' on localPort may have been set as a result from a previously executed XMPFPST, XMPFRCV, XMPFRRH or XMPFRRE call.)
...
- **XFWT F** - Wait flag. If no message is waiting on localPort, the task is suspended. Execution resumes when a message arrives on localPort.
...
**EXAMPLE**: 
```
xmpfrhd(0,msgIdentifier,bytes0To1,bytes2To3,&
...
| 1   | flags Integer        | R   | Options. |
| 2   | localPort Integer    | R   | Number of the receiving port. |
| 3   | userAddress Integer Xmuseraddress | R | Address of user buffer. |
| 4   | userDisp Integer     | R   | Displacement within userAddress in bytes. See note below. |
| 5   | userLength Integer   | R   | Number of bytes you want to read. |
...
If no message is waiting on localPort and the XFWTF flag is not set, the base value of the error codes (XMVENTM) is returned as returnStatus.
...
```
2**XFWTF =: flags
xmpfrfre(flags,localPort,useraddress,userDisp,&
...
|-----|-----------------------------|-----|------------------------------------------------------------------------------|
| 1   | flags<br>Integer            | R   | Options.                                                                    |
| 2   | localPort<br>Integer        | R   | Number of the receiving port.                                               |
| 3   | msgType<br>Integer          | W   | Message type, see explanation below.                                        |
| 4   | remotePort<br>Integer       | W   | Hashed magic number of the remote port.                                     |
| 5   | msgIdentifier<br>XmnsgIdentifier | W   | Message identifier.                                                          |
| 6   | bytesO1olorStat<br>Integer2 | W   | Normally first 2 bytes of message. If msgType is XMTRE, bytesO1olorStat contains the error status. |
...
If no message is waiting on localPort and the XFWTFF flag is not set, the base value of the error codes (XMXENTM) is returned as returnStatus.
...
2**XFWTFF =: flags
  ...
  msgIdentifier,bytesOfIorStat) =: returnStatus
```
...
**XFHIP** - High priority message. The message will be chained to the head of the receiver's queue instead of the tail, following any other high priority messages already queued.
...
**XFBNC** - Bounce message. When the receiver issues 'Receive Message' (i.e., the routines XMPRCV, XMPRFRH or XMPRFRE), which would have led to this message being received, it will instead be returned to the sender.
...
| 3   | userAddress Integer  | R   | Address of user buffer containing the parameters.|
| 4   | userDisp Integer     | R   | Displacement within userAddress in bytes. See note below.  |
...
**Note:** On an ND-100, the `userDisp` is always rounded down to the previous even byte.
...
`XMPFSMC` returns as soon as an XMSG function terminates with status less than or equal to zero (or when all functions have been executed). The return parameters (Treg, Areg, Dreg, and Xreg) are set according to the return registers from the last XMSG function executed. Completion status is also returned in the `returnStatus`.
...
### XFFWD
- **Description:** Forwarding message. The sender information in the message will not be updated. To the receiver, it will appear that the message was sent directly from the previous sending port.

### XFROU
- **Description:** Route message. Ignore the `remoteMagicNum` parameter and send the message to the local routing task (XROUT). The message contents should be parameters to XROUT. (See appendix B on XROUT services.)

### XFRRO
- **Description:** Remote route message. If the XFROU flag is also set, then send the message to a remote routing task (XROUT). The 16 most significant bits of `remoteMagicNum` is assumed to contain the system number to which the message will be sent. The message contents should be parameters to XROUT.
- **Note:** If the XFROU flag is not set and XFRRO is set, the message will be sent as if XFHIP had been set.

### XFHIP
- **Description:** High priority message. If the XFROU flag is not set, the message will be chained to the head of the receiver's queue, instead of the tail, following any other high priority messages already queued.
- **Note:** If both the XFROU flag and the XFHIP flag are set, the message will be sent as if XFROU and XFRRO had been set (i.e., when XFROU is set, setting the XFHIP flag will act as if the XFRRO flag had been set instead).

### XFBNC
- **Description:** Bounce message. When the receiver issues 'Receive Message' (i.e., the routines XMPFRCV, XMPFRRH or XMPFRRE), which would have led to this message being received, it will instead be returned to the sender.
...
### EXPLANATION
After opening a connection port using XMPOPCN, a task can later increment (when the connections become available) or decrement (when the number of connections needs to be reduced) the free connection counter associated with that port.
...
### Example
When a server port with port number 12, which has previously been created using XMPOPCN, is able to handle a new connection, we should inform XROUT.

```
% We are able to handle one more connection
1 =: extraConn
% on the port number returned from xmopcn
12 =: portNumber
...
The maximum port name length accepted by the routine is defined by the symbol `XMMAXNameLength` in the `XMP:DEFS` file. If `portName` is longer than `XMMAXNameLength` in bytes, `-1` will be returned as an error code in `returnStatus`. If the name length exceeds another limit, which is set at XMSG generation time, the port name will be truncated by XMSG, i.e., excess characters are discarded.
...
The maximum port name length accepted by the routine is defined by the symbol `XMMAXNameLength` in the `XMP:DEFS` file. If `portName` is longer than `XMMAXNameLength` in bytes, -1 will be returned as an error code in `returnStatus`. If the name length exceeds another limit, which is set at XMSG generation time, the port name will be truncated by XMSG, i.e., the excess characters are discarded.
...
% Name the port
'torunn' =: portName
% Let xmpoprm do the job
xmpoprm(flags, portName, portNumber) =: returnStatus
...
|-----|----------------------|-----|----------------------------------------------------|
| 1   | flags Integer        | R   | Options.                                           |
| 2   | msgIdentifier Xmsgidentifier | R | Message identifier.                        |
...
| 3   | Xmmsgidentifier       |     |                                   |
|     | localPort Integer     | R   | Number of the sending port.       |

**FUNCTION**: Sends a message to, or via, the local routing task (XROUT).
...
Note that the message contents should be parameters to XROUT. However, if the message contains a letter service request (see XMPELET) which is sent via XROUT, the remainder of the message can contain data for the (remote) receiving (server) task. When a message is sent to another task via XROUT, it is forwarded to the (remote) receiving port as a secure message (i.e., the message is forwarded as if it had been sent with the XFSEC flag set, see the description of XFSEC under the routine XMPFSND).
...
% Check returnStatus, and if Ok, send the message
% from myPort via XROUT to the remote port.
...
<small>Scanned by Jonny Oddene for Sintran Data © 2021</small>
...
| 1   | flags Integer        | R   | Options.                                 |
| 2   | msgIdentifier Xmmsgidentifier Integer | R | Message identifier.                 |
| 3   | bytes0To1 Integer2   | R   | Bytes 0 and 1 of the message header.     |
...
### EXPLANATION
The data will be written into the message buffer specified by msgIdentifier. msgIdentifier will first be set as 'task current' message, then the user data will be written as described under routine XMPFWHD. If msgIdentifier is -1, the currently defined 'task current' message is assumed. 
...
C First, we initialize the user buffer
   returnStatus = xmfbin1(myBuff,lengthBuffer,offSet)
C Check returnStatus, and if Ok,
C append a 32-bit value as parameter 1
   returnStatus = xmfadab(myBuff,offSet,magicNumb,1)
...
| 2   | offset Integer          | IO  | Current number of bytes in the buffer.                   |
...
C append the string as parameter 1
   returnStatus = xmfBfst(myBuffer, offset, portName(1:1), 1)
C Check returnStatus, and if Ok, put in
C the serial number and the service number
   returnStatus = xmfBrdy(myBuff, XSGIN, serialNumber)
```
...
```plaintext
C Specify the total buffer length in bytes.
  lengthBuffer = 100
C Initialize the user buffer
  returnStatus = xmfBini(myBuff,lengthBuffer,offSet)
C offSet no of bytes are used for the XROUT header.
C Check returnStatus, and if Ok, append parameters.
  returnStatus = xmfBast(myBuff,offSet,
                         systName(1:1),1)
C Check returnStatus, and if Ok, we have used offSet
C no of bytes for the XROUT header and parameter 1.
...
```
+---------------------------+--------------------------------+
| Function name : XMFBLLET  | Type: Buffer Formatting        |
+----+---------------------+----+-----------------------------+
| No | Parameter Name/Type | I/O| Explanation                 |
+----+---------------------+----+-----------------------------+
| 1  | headerBuffer        | I  | Local user buffer.          |
|    | Integer*2           |    |                             |
| 2  | lengthBuffer        | I  | Total length of the buffer  |
|    | Integer             |    | in bytes.                   |
| 3  | offset              | O  | Number of bytes used in     |
|    | Integer             |    | headerBuffer after          |
|    |                     |    | formatting.                 |
| 4  | serialNumber        | I  | Reference number.           |
|    | Integer             |    |                             |
| 5  | systemName          | I  | Name of destination system. |
|    | Character           |    |                             |
| 6  | portName            | I  | Name of remote port.        |
|    | Character           |    |                             |
+----+---------------------+----+-----------------------------+
```
...
Note that this function just prepares the letter in a local buffer (`headerBuffer`). It does not copy the header into an XMSG message buffer, nor does it send anything. The data copying and sending must be done using other functions, i.e., XMFWRIT and XMFROUT.
...
When a task (usually a server task) has opened and named a port (e.g., using XMFOPCN or XMFOPNM), another task can now send a message from one of its own ports, via XROUT, to the named (remote) port. The header of the message sent via XROUT must contain a 'Send Letter' (XSLT) service request to XROUT. The remainder of the message can contain user data for the receiving (server) task (e.g., protocol information, user name, password etc.). The remainder of the message will not be looked at by XROUT (i.e., XROUT will only look at the header of the message - the letter), thus the user data can be (coded) in any format legible to the receiving (server) task.
...
When the (server) task receives the message on portName, it can, and it normally will, check that the sending task is allowed to use the server before it sends a (positive) reply to the requester, and thereby gives away its own magic number. If the server task does not want to give away its own magic number, it can do so by sending a (negative) reply with the XFEWD (forward message) option.
...
```
C We are going to send to a port named 's-port' in the
C system named 'scholar'.
  portName = 's-port'
  systemName = 'scholar'
...
C         serialNumber,systemName(0: -1),portName1:-1))
...
C the port name as parameter 1
returnStatus = xmfba st(myBuffer,offSet,
                        portName(1:-1),1)
    
C Check returnStatus, and if Ok, append
C the system name as parameter 2
returnStatus = xmfba st(myBuffer,offSet,
                        systemName(1:-1),2)
    
C Check returnStatus, and if Ok, we will insert
C the serial number and the service number XSLET
...
#### EXPLANATION
This function can be used to close a named port that has been opened and named using XMFOPNM or MFOPCN. The port specified by `portNumber` will be closed and the name assigned to `portNumber` will be cleared (i.e., the name will be removed from XROUT's name table).
...
portName = 'xx-server'
portNumber = 4
returnStatus = xmfclnm(0,portName(1:L-1),portNumber)
```
...
### EXPLANATION:
On return, XMSGpassword contains the password which is needed in order to become a privileged XMSG task (see function XMFFPRV). XMSGrestartCnt returns the number of times XMSG has been (re)started since the last warmstart.
...
### EXAMPLE:
```plaintext
returnStatus = xmfcconf(0,XMSGpassword,configMask,XMSGrestartCnt)
```
...
### Example
```plaintext
returnStatus = xmffabr(0,userBuffer,userDisp,readLength,
                       memoryDisp,bankNumber)
```
...
```plaintext
portNumber = -1
returnStatus = xmmfcls(0, portNumber)
```
...
An nonprivileged task is a task which has not (yet) successfully executed the XMFFPRV call. A nonprivileged system is a remote system which has not (yet) been defined as a friend to your system, see the XROUT service XSDAT.
...
### EXAMPLE
```plaintext
returnStatus = xmffcpv(0,msgIdentifier,accessInfo,additionalInfo)
```
...
+----+------------------+-----+-----------+
| No | Parameter Name/  | I/O | Explanation|
|    | Type:            |     |            |
+----+------------------+-----+-----------+
| 1  | flags            | I   | Options.   |
|    | Integer          |     |            |
+----+------------------+-----+-----------+
| 2  | interruptLevel   | I   | The interrupt level that the driver |
|    | Integer          |     | should run on.                      |
+----+------------------+-----+------------------------------------+
| 3  | registerBlock    | I   | Register block (an 8-word buffer). |
|    | Integer*2        |     |                                     |
+----+------------------+-----+------------------------------------+
| 4  | XTblockAddress   | O   | Address of the XT-block allocated  |
|    | Integer          |     | to the driver.                     |
+----+------------------+-----+------------------------------------+
```
...
|-----|----------------------|-----|-------------------------|
| 1   | flags Integer        | I   | Options.                |
| 2   | bankNo Integer       | I   | Bank number.            |
...
Releases all XMSG resources. All ports opened by the task are closed and all XMSG space belonging to the current caller is released. Special action is taken in the case of current messages, and messages waiting on the input queue (see XMFFSND, XMFFRCV, and XMFFCLS).
...
**EXAMPLE:** 
```plaintext
returnStatus = xmmfdmm(0,requestedTaskSp)
```
...
**EXPLANATION:**  
The call parameter `startScanPort` specifies the last port to be searched. If `startScanPort` is zero, this implies the most recently opened port (i.e., the default port). Note that the search will begin with the next port (if any) after that specified, and then follow the task's port list (see example below).

On return, the parameter `portNumber` contains the port number where the message, if any, is waiting. If no message is waiting and the XWTF flag is not used, the base value of the error codes (XMVENTM) is returned as a status.

For example, if the task has opened four ports and have the port numbers 15 (from the 1st XMFFOPN), 4 (from the 2nd XMFFOPN), 6 (from the 3rd XMFFOPN) and 19 (from the 4th XMFFOPN), then the port list comprises the ports 19-6-4-15 (in that order!). Port number 19 (the first port in the list) is the task's default port (i.e., if `startScanPort` is zero, this port is assumed). If the task has just handled a message received on port 6, it can, when it wants to have a 'round-robin' scheduling of requests, call XMFFGST with `startScanPort=6`. Port 6 will then be the last port to be looked at by XMSG. XMSG will start looking at port 4 to see if a message is waiting. If no message is waiting on port 4, XMSG will look at port 15. If no message is waiting on port 15, XMSG will look at port 19, and, if no message is waiting on port 19, XMSG will finally look at port 6.
...
**XFWAKE** - General wake up. Unless a message is already waiting on one of the ports, a 'general wake up' bit will be set for this task. When 'general wake up' is set, the next transmission to any of the ports opened by the task will lead to a wake up of the receiver task, and clearing of the 'general wake up' bit for that task.
...
**XFHIP** - High priority message. Allows a task to check the arrival of high priority messages. If a high priority message is waiting on one of the ports and XFHIP is set, the port number where the high priority message is waiting is returned in portNumber. If no high priority message is waiting on any of the ports and XFHIP is set, and XFWTFT is not set, the base value of the error codes (XMXENTM) is returned as returnStatus. When the next message of any type is sent to a port opened by the task, the task will be woken up (i.e., if no high priority message is waiting, XFHIP has the same effect as XFWAKE).
...
|-----|----------------------------|-----|---------------------------------------------------|
| 1   | flags Integer              | I   | Options.                                          |
| 2   | magicNumber Integer*4      | I   | Magic number.                                     |
| 3   | portNumber Integer         | O   | Port number.                                      |
| 4   | systemNumber Integer       | O   | System number.                                    |
| 5   | RTinOrOtherInfo Integer    | O   | RT index or other information. See below.         |
| 6   | additionalInfo Integer     | O   | Additional information. See below.                |
...
### EXAMPLE
```plaintext
returnStatus = xmffmst(0,msgIdentifier,msgType,
                       remoteMagicNum,msgLength)
```
...
### EXAMPLE
`returnStatus = xmfpp2m(0,portNumber,magicNumber)`
...
| 1   | flags                   | I   | Options.           |
|     | Integer                 |     |                    |
| 2   | xmsgPassword            | I   | XMSG password.     |
|     | Integer                 |     |                    |
...
- **XFWT** - Wait flag. If no message is waiting on localPort, the task is suspended. Execution resumes when a message arrives on localPort.

- **XFWAK** - Wake up. Unless a message is already waiting on localPort, a 'wake up' bit will be set on localPort. When 'wake up' is set on localPort, the next transmission to this port will lead to a wake up of the receiver task, and clearing of the 'wake up' bit on localPort. This option can be enabled on more than one port at a time.

  However, be aware that if the task is in XMSG wait position (for example, sending a secure message with wait), when the task should have been woken up as a result of a message being sent to localPort, the 'wake up' bit will be cleared but the task will not (and cannot) be woken up.

- **XFHIP** - High priority message. Allows a task to check the arrival of high priority messages. If a high priority message is waiting and XFHIP is set, the message type XMTHI is returned in msgType. If no high priority message is waiting, and XFHIP is set and XFWT is not set, a zero (0) is returned in msgType. When the next message of any type is sent to localPort, the receiving task will be woken up (i.e., if no high priority message is waiting, XFHIP has the same effect as XFWAK).
...
```
returnStatus = xmfppst(flags, localPort, msgType,
                       remotePort, msgIdentifier, queueLength)
```
...
If no message is waiting on localPort and the XFWTF flag is not used, the base value of the error codes (XMXENTM) is returned as returnStatus.
...
- **XFWTF**: Wait flag. If no message is waiting on localPort, the task is suspended. Execution resumes when a message arrives on localPort.
- **XFWAKE**: Wake up. Unless a message is already waiting on localPort, a 'wake up' bit will be set on localPort. When 'wake up' is set on localPort, the next transmission to this port will lead to a wake up of the receiver task, and clearing of the 'wake up' bit on localPort.
...
| 3   | inbuffer Integer     | I   | User buffer.                                      |
| 4   | userDisp Integer*2   | I   | Displacement within inBuffer in bytes. See note below. |
...
**Example:**  
`returnStatus = xmffrea(0,msgDisp,inbuffer,userDisp,  
userLength,readLength)`
...
If no message is waiting on localPort and the XFWTF flag is not set, the base value of the error codes (XMNEATM) is returned as returnStatus.
...
| 2   | localPort Integer    | I   | Number of the receiving port. |
| 3   | msgType Integer      | O   | Message type, see explanation below. |
| 4   | remotePort Integer   | O   | Hashed magic number of the remote port. |
| 5   | msgIdentifier Integer | O  | Message identifier. |
| 6   | bytesOToIorStat Integer*2 | O | Normally first 2 bytes of message. If msgType is XMTRE, bytesOToIorStat contains the error status. |
...
flags = 2**XFWTF
returnStatus =
   xmfrrh(flags,localPort,msgType,
...
### Options
- **XFWTWTF** - Wait flag. This is only significant when sending a secure (XFSEC) message to a task in another system.
...
| 2   | XMSGbase <br> Integer| O   | XMSG base field address.          |
...
```plaintext
returnStatus = xmffsmc(0,noOfcalls,userBuffer,
                       userDisp,Treg,Areg,Dreg,Xreg)
```
...
When a task wants to send a message to another task, it must know the magic number of a port of the other task. A description of how to obtain the magic number is given in the sample programs and under the XMFBLT call. A remoteMagicNum parameter of -1 will direct the message back to the port from which it was last sent.
...
**XFWTFF** - Wait flag. This is only significant when sending a secure (XFSEC) message to a task in another system.
...
### Example
```plaintext
returnStatus = xmfstd(0,XTblockAddress)
```
...
| No: | Parameter Name/Type: | I/O | Explanation:                           |
|----|----------------------|-----|---------------------------------------|
| 1  | flags Integer*2      | I   | Options.                              |
| 2  | bytes0To1            | I   | Bytes 0 and 1 of the message header.  |
|    | Integer*2            |     |                                       |
| 3  | bytes2To3            | I   | Bytes 2 and 3 of the message header.  |
|    | Integer*2            |     |                                       |
| 4  | bytes4To5            | I   | Bytes 4 and 5 of the message header.  |
|    | Integer*2            |     |                                       |

**FUNCTION**: Writes to the header of the 'task current' message buffer.

**EXPLANATION**: If the 'whole-message-read' flag has been set (see XMFFREA), it is cleared and the current message length (not the same as size) is set to 0. Then the function inserts bytes0To1, bytes2To3, and bytes4To5 as the first six bytes of the message. If this results in the message being longer than before, the current message length is set to 6. It then sets the current message displacement to 6.
...
**EXAMPLE**: `returnStatus = xmffwhd(0, bytes0To1, bytes2To3, bytes4To5)`
...
+----+------------------+--------+---------------------+
| 1  | flags Integer    | I      | Options.            |
| 2  | msgDisp Integer  | I      | Message displacement|
|    |                  |        | (within XMSG buffer)|
|    |                  |        | in bytes.           |
| 3  | outbuffer        | I      | User buffer.        |
|    | Integer*2        |        |                     |
| 4  | userDisp Integer | I      | Displacement in the |
|    |                  |        | user buffer, in     |
|    |                  |        | bytes. This must be |
|    |                  |        | an even number.     |
| 5  | userLength       | I      | Number of bytes you |
|    | Integer          |        | want to write.      |
| 6  | writtenLength    | O      | Number of bytes     |
|    | Integer          |        | actually written.   |
+----+------------------+--------+---------------------+
```
...
```plaintext
returnStatus = xmffwri(flags,msgDisp,outbuffer,userDisp,
                       userLength,writtenLength)
```
...
### Function Name: XMF1NFC

| No: | Parameter Name/ Type: | I/O | Explanation:                        |
|-----|-----------------------|-----|-------------------------------------|
| 1   | flags                 | I   | Options.                            |
| 2   | portNumber Integer    | I   | Port number returned by XMPOPPCN.   |
...
### EXPLANATION
After opening a connection port using XMPOPPCN, a task can later increment (when connections become available) or decrement (when number of connections need to be reduced) the free connection counter associated with that port.
...
When a server port with port number 12, which has previously been created using XMFOPCN, is able to handle a new connection, we should inform XROUT.
...
C To recognize the reply, we need a reference no.
   serialNumber = 100
C Tell XROUT
   returnStatus = xmfinc(0,portNumber,extraConn,
                         serialNumber)
```
...
returnStatus = xmfopen(0, portName(1:-1), uniqueName,
...
|-----|-----------------------|-----|------------------------|
| 1   | flags<br>Integer      | I   | Options.               |
| 2   | portName<br>Character | I   | Name of the port.      |
| 3   | portNumber<br>Integer | O   | Port number.           |
...
```plaintext
returnStatus = xmfread(flags, msgIdentifier, msgDisp,
                       inbuffer, userDisp, userLength, readLength)
```
...
Sends a message to, or via, the _local_ routing task (XROUT).
...
This example creates a letter, writes the letter into an XMSG buffer, fills in data for the receiving task, and sends the message via XROUT using XMFRTOUT to a remote (server) port.
...
C named 'zz-port' in the system named 'gokk'.
    portName = 'zz-port'
    systemName = 'gokk'
C Let xmfblet create the letter in our local buffer.
    returnStatus = xmfblet(myBuffer,60,offset,123,
                  systemName(l:-1),portName(l:-1))
C Check returnStatus, and if OK, let's
C reserve an XMSG buffer of 200 bytes.
    returnStatus = xmmfget(0,200,msgIdent)
C Check returnStatus, and if OK, copy the letter
C created by xmfblet into the XMSG buffer.
    uLength = offset(l:-1)
    returnStatus = 
    xmfwrfi(0,0,myBuffer,0,uLength,wLength)
C Check returnStatus, and if OK, write data for
C the receiving server task into the XMSG buffer.
    returnStatus = 
    xmfwrfi(0,wLength,serverData,0,50,offset)
C Check returnStatus, and if OK, open a port
C so that we can send the message.
    returnStatus = xmfeopn(0,myPort)
...
| 1   | flags                  | I   | Options.                             |
|     | Integer                |     |                                      |
| 2   | msgIdentifier          | I   | Message identifier.                  |
|     | Integer                |     |                                      |
| 3   | localPort              | I   | Number of sending port.              |
|     | Integer                |     |                                      |
| 4   | remoteMagicNum         | I   | Magic number of receiving port.      |
|     | Integer*4              |     |                                      |
...
**EXPLANATION**: 
The data will be written into the message buffer specified by msgIdentifier. msgIdentifier will first be set as 'task current' message. Then the user data will be written as described under function XMFWRHD. If msgIdentifier is -1, the currently defined 'task current' message is assumed.

**OPTIONS**: As for XMFWRHD.

**RULES**: Permitted for both non-privileged and privileged tasks.

**EXAMPLE**: 
```plaintext
returnStatus = xmfwrhd(0,msgIdentifier,bytes0To1,
                       bytes2To3,bytes4To5)
```
...
```plaintext
returnStatus = xmfwrtf(flags,msgIdentifier,msgDisp,
                       inbuffer,userDisp,userLength,writtenLength)
```
...
The only way an event can be signalled to you is through the RRPBWAIT call. Only one event can be signalled in one RRPBWAIT call.
...
The RRPBWAIT routine is at some point performing the 'tmout' monitor call. If your RT-program becomes rescheduled for execution, for example by another program, the RREVohr will occur.
...
|-----------|-----------------------------|-------------------------------|
| RRPSABRT  | Abort connection            | Disconnect                    |
| RRPSDCIN  | Disconnect indication       | Disconnect                    |
| RRPSDCRQ  | Disconnect request          | Disconnect                    |
| RRPBINIT  | Initialize RR-LIB           | Used in all phases            |
| RRPBWAIT  | Wait for event              | Used in all phases            |
| RRPSSCNI  | Connection indication       | Connection establishment      |
| RRPSSCNR  | Connection response         | Connection establishment      |
| RRPSSEND  | Clear up untidy ends        | When finished                 |
| RPPSGTRQ  | Get request                 | Data transfer                 |
...
| Routine   | Purpose                     | Phase where used              |
|-----------|-----------------------------|-------------------------------|
| RRPCABRT  | Abort connection            | Disconnect                    |
| RPCDCIN   | Disconnect indication       | Disconnect                    |
| RPCDCRQ   | Disconnect request          | Disconnect                    |
| RRPCINIT  | Initialize RR-LIB           | Used in all phases            |
| RRPCWAIT  | Wait for event              | Used in all phases            |
| RRPCCNCF  | Connection confirmation     | Connection establishment      |
| RRPCCNRQ  | Connection request          | Connection establishment      |
| RRPCEND   | Clear up untidy ends        | When finished                 |
...
| 2 | remoteID Rrid | R | ID returned from RRPSnmin. |
...
TYPE RRSP = RECORD
    BOOLEAN: RRSPisDefault  % TRUE => default
                           ...
                           % FALSE => must fill in
                           % remainder of record
    INTEGER: RRSPlallocation % requested value of XMSG
                             % task space
...
RRSPlallocation is the requested value of the XMSG task space.
...
```plaintext
CONSTANT RRMXevData = 2   % Length of the associated event data.
```
...
```plaintext
RRPBwait(static,timeout,requestedEvent,
...
**EXPLANATION**: 
This call gives you information from the client after an RREVCnin event.

The SIZE of the serverInfoBuff gives you the maximum amount of data that may be returned to the client.

**USAGE**: 
RRPSenin(static, remoteID, clientInfo, serverInfoBuff) =: RRstatus
...
### USAGE:
```
RPPSgtrq(static, remoteID, request, responseBuffer)&
=: RRstat
```
...
| 1   | static               | R   | Fixed size work space.                                 |
|     | Integer array        |     |                                                        |
| 2   | remoteID             | R   | ID returned in actualEvent when the RREVrqin event occurred. |
|     | Rrid                 |     |                                                        |
| 3   | response             | R   | Response to the client, placed in the buffer given by RRPSgtrq. |
|     | Bytes                |     |                                                        |
...
**USAGE**: 
```
RRPBinit(static,dynamic,mode,maxClientConn,
         serverName,bufferArea,maxServerConn,
         serverParam) =: RRstatus
```
...
| No: | Parameter Name/Type: | R/W | Explanation:                                            |
|-----|-----------------------|-----|---------------------------------------------------------|
| 1   | static Integer array remoteID | R   | Fixed size work space.                                   |
| 2   | Rrid                 | R   | The identifier returned by RRPCSLCT.                    |
...
| 6   | response Bytes pointer | W   | Server response.                                        |
| 7   | reason Integer       | W   | Reason for the disconnection or unexpected event value. |
...
```
RRPCall(static, remoteID, request, responsebuffer,
        timeout, response, reason) =: RRstatus
```
...
| 6   | timeout Rtm          | R   | Maximum waiting time for response from the server.                                                                                              |
...
This call is equivalent to `RRPCncrq+RRPBwait+RRPCcncf/RRPBdcin`. This is done synchronously, i.e., the return does not occur until timeout or until a response is received from the server.
...
Connection setup overhead is reduced if `clientInfo` is also used to include the first request to server and `RRPCsnrs` is used to return the response.
...
```
RRPCsIct(static,destSys,destServer,clientInfo,
         serverInfoBuff,timeout,remoteID,serverInfo,
         reason) =: RRstatus
```
...
**FUNCTION:** Disconnect request with wait for completion.

**EXPLANATION:** This call is equivalent to RRPBdcrq+RRPBwait.
...
|     | Integer array        |     |                                   |
| 2   | remoteID             | R   | ID returned from RRPCcnrq.        |
|     | Rrid                 |     |                                   |

**FUNCTION**: Abort connection.

**EXPLANATION**: This end of a connection is aborted. You should use this call only if a attempt to disconnect RRPDdcrq does not work, i.e., if you do not get a confirmation after a reasonable time.
...
|-----|-----------------------|-----|-----------------------------------------------|
| 1   | static                | R   | Fixed size work space.                        |
| 2   | Integer array remoteID| R   | ID returned in actualEvent when disconnect was signalled. |
...
**USAGE:** `RRPBinit(static, dynamic, mode, maxClientConn, serverName, bufferArea, maxServerConn, serverParam) =: RRStatus`
...
- **CONSTANT**: RRMXevData = 2
  - % Length of the associated event data.
...
| 1   | static                        | R   | Fixed size work space.                                       |
|     | Integer array                 |     |                                                              |
| 2   | destSys                       | R   | String identifying the system where the                      |
|     | Bytes                         |     | server resides.                                              |
| 3   | destServer                    | R   | Server name.                                                 |
|     | Bytes                         |     |                                                              |
| 4   | clientInfo                    | R   | Information from client to server (for                       |
|     | Bytes                         |     | example an identification).                                  |
| 5   | serverInfoBuff                | R   | Buffer in which to place information                         |
|     | Bytes pointer                 |     | from server to client.                                       |
| 6   | remoteID                      | W   | Server reference number to be used in                        |
|     | Rrid                          |     | subsequent calls.                                            |
...
**FUNCTION:** Ends the client functions.
...
```
RRPCsnrq(static, remoteID, request, responseBuffer)
= : RRstatus
```
...
Example of call: `CALL TLFTMLS(TLIBREF,ISTAT)`

Normal return status is zero. If you include the appropriate definition file in your source code, you may use the symbol OK for the zero status. The file is called TLP:DEFS for PLANC and TLF:DEFS for FORTRAN. Other return statuses are error codes. A list of the error codes plus their corresponding symbols is provided in appendix F.
...
| TLEVncf    | 4     | Connection is accepted                     |
| TLEVdain   | 8     | Data has arrived                           |
| TLEVdrt    | 16    | Credit change                              |
...
| TLEVdcef   | 256   | Requested disconnection is completed       |
| TLEVunkn   | 1024  | Message arrived on port not known to TLIB  |
| TLEVothr   | 2048  | Not a TLIB event, timeout, or unknown port |

The reason for TLEVunkn is that you may combine the use of TLIB and XMSG in the same RT program. As you performed the TLWAIT call, to wait for an event, a message may arrive on a port opened by an XMSG call. The TLEVunkn tells you that this has happened.

The TLPTWAIT routine is at some point performing the `tmout` monitor call. If your RT-program becomes rescheduled for execution, for example by another program, the TLEVothr event will occur.
...
When you have provided TLIB with a buffer, TLIB "owns" this buffer, although it still physically resides in your program area. Such a buffer always becomes allocated to a specific connection. To get the buffer back from TLIB with valid data, you have to do one of the following calls:
    ...
    TLPCNIN, TLPCNCF, TLPDAIN, TLPEDIN, or TLPCDIN
...
If a connection indication, a connection confirmation, or a disconnect indication contains user data, then the first, sufficiently large buffer provided, following the event, is used for the user data. In the presence of user data, such a buffer must be provided between the event and the call TLPCNIN, TLPCNCF, or TLPCDIN.
...
| TLPTRMLS  | Terminate listening       | Connection establishment  |
...
| client program | server program | Description |
|----------------|----------------|-------------|
| TLPCNRQ        | TLPCNIN        | % connect to remote user % get info from connection % indication event |
| TLPWAIT        | TLPCNRS        | % wait for acceptance % accept connection |
| TLPCNCF        | TLPCNRS        | % confirmation event % connection now established % and data may be transferred |
| TLPPBBF        | TLPPBBF        | % only necessary if % user-data was present % get data from connection |

### Data Transfer - One or More Instances of the Following Sequence

| client or server program | other program | Description |
|--------------------------|---------------|-------------|
| TLPDARQ                  | TLPWAIT       | % transfer initiated by % either user % by one or more calls |
|                          | TLPPBBF       | % this could be done % before data indication % by one or more calls |
|                          | TLPDAIN       | % get data % by one or more calls |

### User-Initiated Termination of Established Connection

| client or server program | other program | Description |
|--------------------------|---------------|-------------|
| TLPDCRQ                  | TLPWAIT       | % termination initiated by % either user |
|                          | TLPDCIN       | % get info from disconnect % indication event |
...
[Page number and document details: 212, Scanned by Jonny Oddene for Sintran Data © 2021]
```
...
I'm unable to transcribe or recreate the content of this page as it appears to be a non-text cover page with only general information like page number, title, and a reference to the document. There's no specific technical content or diagram included on this page.
...
|--------------------------|---------------------------------------------------------------------------------------------------------------|
| `TLEVdain`               | `TLEVdata(0)` gives the total amount of data received since the last signalled event of this type.<br>`TLEVdata(1)` gives the number of complete TSDUs in the total data. |
| `TLEVxdin`               | `TLEVdata(0)` gives the length of the expedited data.                                                         |
| `TLEVenin`<br>or `TLEVcncf`<br>or `TLEVdcin` | `TLEVdata(0)` gives the length of the user data.                                                           |
| `TLEVcrdt`               | `TLEVdata(0)` gives the new value of the user's credit.                                                       |
| `TLEVunkn`               | `TLEVdata(0)` gives the XMSG port number.<br>In this case, TLEVrefno has no significance.<br>If the TLEVunkn event occurs, the return from TLPWAIT is equivalent to a return from the XMSG general status call (XMPFGST).<br>Note that the message must be received before TLPWAIT is called again. |
...
### USAGE
```plaintext
tlpcnin(tlib_refno, retstat, remote_address, quality&
        initial_credit, userdata, length_userdata)
```
...
| No. | Parameter Name/Type | R/W | Explanation                           |
|-----|---------------------|-----|---------------------------------------|
| 1   | user_address        | R   | Local TLAP.                           |
|     | TLap                |     |                                       |
| 2   | remote_address      | R   | Remote TLAP.                          |
|     | TLap                |     |                                       |
| 3   | user_refno          | R   | Connection identifier belonging to user. |
|     | Integer             |     |                                       |
| 4   | quality             | R   | Quality of service.                   |
|     | Tlqs                |     |                                       |
| 5   | userdata            | R   | User data. Must start on a word boundary. |
|     | Tlbf                |     |                                       |
| 6   | length_userdata     | R   | Length of user data. Must be an even number. |
|     | Integer             |     |                                       |
| 7   | retstat             | W   | Return status.                        |
|     | Integer             |     |                                       |
| 8   | tlib_refno          | W   | Connection identifier belonging to TLIB. |
|     | Integer             |     |                                       |
...
A connection between the local and the remote TLAPs is initiated. Before the exchange of TSDUs can take place, the connection must be completely established.

## Usage

```
tlpcnrq(user_address,remote_address,user_refno,quality,&
...
**USAGE:** `tlpcnrs(tlib_refno,quality,userdata,length_userdata,& retstat)`
...
When data is received, it is signalled via the event TLEVdain. If TLIB is not yet provided with a data buffer, you must perform a TLPPRBF call. Then you may, at any time, obtain the data buffer from TLIB.
...
### USAGE
tlpdarq(tlib_refno,user_buffer,length_data,&  
flags,retstat,new_credit)
...
### EXPLANATION
An incoming disconnect request is perceived by TLIB as a disconnect indication. TLIB signals this to you via a TLEVCdcin event. TLPDCIN gives you the information associated with this event.

### USAGE
```
tlpdcin(tlib_refno, retstat, disconn_reason, userdata, length_userdata)
```
...
| 1   | tlib_refno Integer   | R   | Connection identifier belonging to TLIB.        |
| 2   | retstat Integer      | W   | Return status.                                  |
| 3   | user_buffer Tlibf    | W   | Buffer provided for expedited data.             |
...
```plaintext
tlpedin(tlib_refno,retstat,user_buffer,&
        length_expdata)
```
...
Only one unit of expedited data may be outstanding at a time. You may send no more until you receive a TLEVxedt event.

### USAGE
```plaintext
tlpedrq(tlib_refno,user_buffer,length_expdata,&
        retstat)
```
...
**TLIB operating mode** is specified by the TLMD record which is defined in the TLP:DEFS file:
...
|----|---------------------|-----|--------------------------------------------------|
| 1  | tlib_refno Integer  | R   | Connection identifier belonging to TLIB.         |
| 2  | user_buffer Tlf     | R   | User buffer. Must start on a word boundary.      |
...
tlppprbf(tlib_refno, user_buffer, &  
length_buffer, retstat)
...
# COSMOS PROGRAMMER GUIDE TLIB/PLAN C REFERENCE GUIDE

## Routine name: TLPSTLS

| No: | Parameter Name/ Type: | R/W | Explanation:                                   |
|-----|-----------------------|-----|------------------------------------------------|
| 1   | user_address <br> Tlap | R   | Local TLAP.                                    |
| 2   | user_refno <br> Integer| R   | Connection identifier belonging to user.        |
| 3   | retstat <br> Integer   | W   | Return status.                                 |
| 4   | tlib_refno <br> Integer| W   | Connection identifier assigned by TLIB.         |
...
**USAGE:** `tlptmls(tlib_refno, retstat)`
...
|     | TlTm                  |     |                                 |
| 2   | requested_event       | R   | Requested event.                |
|     | Tlev                  |     |                                 |
| 3   | retstat               | W   | Return status.                  |
|     | Integer               |     |                                 |
| 4   | actual_event          | W   | Actual event.                   |
|     | Tlev                  |     |                                 |

**FUNCTION**: Waits for event to occur.

**EXPLANATION**: See also important note on next page.
...
```
TYPE TLTMcode = ENUMERATION (TLTMbasic, TLTMsecs, 
                             TLTMmins, TLTMhrs)
```
...
The actual event that has occurred, is returned in actual_event, with `<actual_event.TLEVrefno>` set to a user_refno.

### USAGE

```
tlpwait(timeout,requested_event,retstat,&
        actual_event)
```
...
All the symbols referred to, except array names like IRMADD and IEVENT, are defined in the TLF:DEFS file. You should include it in your program.
...
TLAPLNNET = 1
TLAPNETADDRESS = 2
TLAPPLUSSUFFIX = 2+(TLMXNAMELENGTH/4)
TLAPSUFFIX = 3+(TLMXNAMELENGTH/4)
TLAPMAGIC = 1
```

IRMADD(TLAPFORMAT) specifies the format in which the address is given. There are two possible formats: TLTAP2STRING or TLTAPMAGIC.

If IRMADD(TLAPFORMAT) = TLTAP2STRING, then the address is contained within the array as two character strings:

```
IRMADD(TLAPLNNET) gives the number of
characters in the "network address". The "network
address" is the same as the system name.
IRMADD(TLAPNETADDRESS), IRMADD(TLAPNETADDRESS+1)....
contain the packed character string for the network
address.
```
...
The quality of service is specified by an integer array. It may be dimensioned as follows: DIMENSION IQOS(0:TLAIQOS-1) where TLAIQOS = 1. Only the default quality of service is currently provided. One way of setting the default value is: IQOS(TLQOSDEFAULT) = TLISDEFAULT.
...
```plaintext
[Photo: Technical document page with text and symbols detailed as per the sections above]
```
...
| (TLEVCODE)                  | TLEVODATA                                                                 |
|-----------------------------|---------------------------------------------------------------------------|
| = TLEVDATN                  | (TLEVODATA) gives the total amount of data received since the last signalled event of this type. (TLEV1DATA) gives the number of complete TSDUs in the total data.        |
| = TLEXVDIN                  | (TLEVODATA) gives the length of the expedited data.                        |
| = TLEVCNIN, TLEVCNCF,       | (TLEVODATA) gives the length of the user data.                            |
| = TLEVCIN                   |                                                                           |
| = TLEVCRDT                  | (TLEVODATA) gives the new value of the user's credit.                     |
| = TLEVUNKN                  | (TLEVODATA) gives the XMSG port number. In this case TLVREFNO has no significance. If the TLEVUNKN event occurs, the return from TLFWAIT is equivalent to a return from the XMSG general status call (XMFGSST). Note that the message must be received before TLFWAIT is called again. |
...
### USAGE:
CALL TLFCNCF(ITLREF, ISTAT, IQOS, ICREDT, IUBFID, LENDAT)
...
| No: | Parameter Name/Type      | I/O | Explanation                                   |
|-----|--------------------------|-----|-----------------------------------------------|
| 1   | IUSADD<br>Integer*4 array| I   | Local TLAP.                                   |
| 2   | IRMADD<br>Integer*4 array| I   | Remote TLAP.                                  |
| 3   | IUSREF<br>Integer        | I   | Connection identifier belonging to user.      |
| 4   | IQOS<br>Integer array    | I   | Quality of service.                           |
| 5   | IUBUF<br>Integer*2 array | I   | User data. Must start on a word boundary.     |
| 6   | LENDAT<br>Integer        | I   | Length of user data. Must be an even number.  |
| 7   | ISTAT<br>Integer         | O   | Return status.                                |
| 8   | ITLREF<br>Integer        | O   | Connection identifier belonging to TLIB.      |
...
**USAGE:** 
```
CALL TLFCNRS(ITLREF, IQOS, IUBUF, LENDAT, ISTAT)
```
...
When data is received, it is signalled via the event TLEVDATIN. If TLIB is not yet provided with a data buffer, you must perform a TLFPRBF call. Then you may, at any time, obtain the data buffer from TLIB.

When it is full, TLIB puts the data buffer in a queue for eventual transfer to you. This transfer from TLIB is initiated when you perform the TLFDAIN call. A user buffer is considered to be full when it is actually filled with data, or when it contains data representing the end of a TSDU. The user will not receive a buffer containing data from more than one TSDU.
...
### Explanation
An incoming disconnect request is perceived by TLIB as a disconnect indication. TLIB signals this to you via a TLEVCIN event. TLFDCIN also specifies the information associated with this event.
...
No other calls may be invoked on the connection after you have done a TLFDCRO.

### USAGE

**CALL TLFDCRO(ITLREF, IUBUF, LENDAT, ISTAT)**
...
|     | Integer             |     |             |
| 2   | IUBUF               | I   | User buffer. Must start on a word boundary. |
|     | Integer*2 array     |     |             |
| 3   | LENDAT              | I   | Number of bytes in user buffer. Must be even. |
|     | Integer             |     |             |
| 4   | ISTAT               | O   | Return status. |
|     | Integer             |     |             |
...
| No: | Parameter Name/Type: | I/O | Explanation:                             |
|-----|----------------------|-----|------------------------------------------|
| 1   | MODE                 | I   | Specification of TLIB operating mode.   |
|     | Integer array        |     |                                          |
| 2   | IDYNAM               | I   | Working storage for TLIB.                |
|     | Integer array        |     |                                          |
| 3   | ISTAT                | O   | Return status.                           |
|     | Integer              |     |                                          |
...
### MODE(TLMDMUNIQUESUFFIX)

MODE(TLMDMUNIQUESUFFIX) is set to TLISUNIQUE, if suffix names should be unique to this task. Otherwise it is set to TLNONUNIQUE.
...
CALL TLFPRBF(TILREF, IUBUF, LENUBF, IUBFID, ISTAT)
...
If you refuse to accept further connection indications, TLFMTLS has to be invoked.
...
- TLTLENGTH = 1, which gives the time length.

The different values that ITMOUT(TLTMUNITS) can have are:

- TLTMBASIC, which means basic time units.
- TLTMSECS, which means seconds.
- TLTMINS, which means minutes.
- TLTNHRS, which means hours.

ITMOUT(TLTLENGTH) describes the number of time units.

A value of TLINFINITETIME for ITMOUT(TLTLENGTH) specifies an infinite timeout period.

A value of 0 for ITMOUT(TLTLENGTH) is equivalent to a poll for outstanding TLIB events of the requested type, on the requested connection.
...
You may specify that TLIB should wait on a particular connection for a particular set of events. This is done by appropriately setting the desired values in IREQEV(TLREVREFNO) and IREQEV(TLREVCODE). IREQEV(TLREVREFNO) requires a connection identifier belonging to TLIB.

The set of desired events is formed from the logical OR of the corresponding event codes.

The value TLANYREFNO may be used to accept any connection. Likewise, TLANYEVENT may be used to accept any event.

The actual event that has occurred is returned in IACTEV, with IACTEV(TLREVREFNO) set to a user's connection identifier.
...
The reason that an "other" event may be missed, is that all wakeups from the wait state use one single bit in the program's RT-description (the SREP bit). If two wakeups, one leading to an "other" event followed by one done by XMSG, occur close together just before TLIB executes the TMOUT monitor call, the information about the "other" event is lost.
...
[Placeholder for any visuals on the page]
...
This appendix gives a description of how to execute XMSG functions using the SINTRAN monitor call MON 200 (MON XMSG). It should also be of interest to the user that intend to use the XMPFSMC routine (PLAN C) or the XMFFSMC routine (FORTRAN).
...
XTADDR - Task block (KT-block) address.
...
```
T:=XFPST (BONE XFWTW/XFWAK/XFHIP)  % T=function BONE options
...
The message has come from a known partner. The X register always contains the queue length, i.e., the number of messages queued for the port.
...
| XMTNO  | Normal message                                                                      |
| XMROU  | Message last sent by XROUT (routing program)                                        |
| XMTIH  | High priority message (sent with XFHIP option)                                      |
| XMTRE  | Returned message (sent as secure, but could not be delivered)                       |

### Options:

If no message is waiting on portNo and the XFWT (wait flag) is set, the task is suspended until the next message arrives on portNo. If the XFWT is not set, a zero status is returned. Then a 'wake up' bit will be set on portNo, if XFWAK (wake up) is set.
...
The XFHIP (high priority message) option allows a task to check the arrival of high priority messages. If the XFHIP option is set and a high priority message is waiting, the message type XMTIH is returned in the T register. If no high priority message is waiting on portNo and if XFHIP is set, and XFWT is not, a zero status is returned in the T register. When the next message of any type arrives, the task will be woken up (i.e., if no high priority message is waiting, XFHIP has the same effect as XFWAK).
...
This function will release all XMSG resources reserved by the task. All the ports opened by the task are closed. All XMSG space belonging to the current caller is released. Special action is taken in the case of current messages, and messages waiting on the input queue (see XFSND, XRFCV and XFCLS).
...
Buffers defined in this way cannot be sent to other systems.
...
|------------|------------------------------------|
| T:=XPFMR   | (BONE XFEXC)                       |
| X:=NMESS   | X=no. of allocated messages to free|
| *MON XMSG  |                                    |
| T:=ISTAT   | T=result status                    |
| A:=NMESS   | A=no. of allocated messages freed  |
...
```
T:=XFPSND (BONE options)  % T=function BONE options (see below)
...
- Bounce message. When the receiver issues 'Receive Message' (i.e., XFRCV, XFRRH or XFRRE), which would have led to this message being received, it will instead be returned to the sender. 
...
DATAX∅::D                 % D=first two bytes of message header
A::=MESAD                % A=message identifier
X::PORTNO                % X=number of sending port
*MON XMSG
T::ISTAT                 % T=result status
...
This leads to MESAD being set as the 'task current' message and the 'port current' message for portNo, DATAX∅ being written into the first two bytes of the message buffer. Then the message buffer (MESAD) will be returned to the port from which it was last sent.
...
- **XFWT** - Wait flag. This is only significant when a secure message (XFSEC) is sent to another system (see below). If set, it implies that the caller will only be restarted (with proper status) when the message has been put into the receiver's input queue (i.e., the sending task is suspended until the message has been sent to the remote port). If not set, secure messages that cannot be delivered, will be returned to the sending port.
...
- **XFWD** - Forwarding message. The sender information in the message will not be updated. Thus, it will appear to the receiver that the message was sent directly from the previous sending port.
...
```
T:=XFRCV (BONE XFWT/FXFWAK)  % T=function BONE options
...
Note that calling this function when a message is waiting on portNo, will lead to the clearing of both the 'general wake up' bit for the task and the 'wake up' bit on portNo. If no message is waiting on portNo, XMSG will clear both the 'general wake up' bit for the task and the 'wake up' bit on portNo, before checking the requested option(s). (The 'general wake up' bit may have been set as a result from a previously executed XFGST function, and the 'wake up' bit on portNo may have been set as a result from a previously executed XFPST, XFRCV, XFRHR or XFRE function.)
...
| Code            | Description                                  |
|-----------------|----------------------------------------------|
| T:=XFRRH        | % T=function BONE options                    |
| A:=PORTNO       | % A=number of receiving port                 |
| *MOM XMSG       |                                              |
| T:=METYP        | % T=message type or result status <=0        |
| A:=RPORT        | % A=hashed magic number of remote port       |
...
NCALLS is the number of functions to be executed. UADD is the address of a buffer containing the parameters for these functions. Each set of parameters comprise 4 words (T, A, D and X registers), so the buffer length should be 8*NCALLS bytes long. NCALLS has a maximum size system dependent, and defined when the XMSG system is generated. If NCALLS is 0 (or -1), then the previous request for multi-calls will be re-executed.
...
### 4.4 Convert Magic Number to Port and System Number (XFMP2P)
...
# 4.6 Define Wake Up Context (XFWUDF)
...
```
T:=XFWUDF       % T=function
DBREG:=B        % B=driver's B register on restart
...
A:=D::AINFO     % D=additional information
```
...
*MON XMSG
T:=ISTAT          % T=result status
A:=BASEAD         % A=XMSG base field address 
                  % (B register)
```
...
I'm unable to transcribe the content of this image.
...
The calls in this manual marked with 'XROUT Services', do not require any formatting of the messages to XROUT. In these cases the necessary formatting automatically is performed by the routines called. However, certain service functions, like 'Get Name from Magic Number' (XSGMN), require that you format the messages yourself. This can be done using the routines marked with 'Buffer Formatting'. Even so, you still need to know the XROUT message format.

You should note that many of the XROUT services are privileged. This implies that they can only be successfully executed by a privileged task. See the XMSG call XMPFPRV (Planc) or XMPFPRV (Fortran).
...
## 3.6 Return a Null Status Message (XSNULL)
...
| Out: 2       | String  | Port or System name                           |
| Out: 3       | Integer | No of free connection/service points (optional) |
...
|--------------|--------|----------------------------------|
| In:          |        |                                  |
| 1            | Integer| Magic number of server port      |
| 3            | Integer| Number of buffers to allocate    |
| 4            | Integer| Not equal to zero if WAN         |

The server magic number must have been obtained previously by direct communication with the server, e.g., using the XSLEX service. The WAN flag is used to indicate that communication over this server is not free. Thus all letters that are sent with the 'LAN only' flag should stop here (cf. XSLET service).
...
When the restart-files are defined and auto-restart enabled, XROUT will, if an XMSG crash occurs, append the files specified as parameter 2 and 3 to the batch input and output queues respectively.
...
The sub-service is used to dump XMSG onto files (for post-use analysis). This sub-service is normally accessed via the Dump-XMSG command of the XMSG-COMMAND program, and when XMSG is dumped in the case of an XMSG crash. When accessed, XROUT will dump XMSG onto the files defined via sub-service XSDDF (see above).
...
This service is also divided into sub-services. The requested sub-service is specified by the first parameter in the message sent to XROUT. The number and type of the other parameters in the message, depends on the requested sub-service.
...
The List-Friend-Systems command of the XMSG-COMMAND program allows one to list all systems defined as friends to the local system. This is done by sending a message with the object system number as parameter 2. The return message will contain two parameters.
...
This privileged service is also divided into sub-services. The requested sub-service is specified by the first parameter in the message sent to XROUT. The number and type of the other parameters in the message, depends on the requested sub-service. XSDAT is normally accessed via the commands of the XMSG-COMMAND program.
...
| SEC Symbol | SEC Value: | XMSG Symbol: | XMSG Value: |
|------------|------------|--------------|-------------|
| XMXEPRV    | 16920      | XEPVR        | -24         |

**Description:** Privilege request refused.  
**Explanation:** To become privileged, the caller must be either a driver, direct task, foreground program on ring 2 or 3, or a background program logged in as user SYSTEM. The caller must also specify the correct XMSG password. The XMSG password can be obtained using the XMPCONF (XMTCONF) routine.

---

| SEC Symbol | SEC Value: | XMSG Symbol: | XMSG Value: |
|------------|------------|--------------|-------------|
| XMXSERNA   | 16921      | XERNA        | -25         |
...
**Explanation:**  
XMSG is not started or XMSG has crashed! When XMSG is running, it spends a lot of its time checking itself. If one of these checks fails, XMSG will restart active XMSG tasks by means of this return status and close itself down.
...
**Explanation:** A 'Start Link' or 'Start Network Server' service call has been issued. When an inter-system link (i.e., HDLC or megalink) or a virtual link (i.e., link to a network server) is started, a (user) specified number of XD-blocks (frame blocks) will be allocated for that particular link. If it is a virtual link, the same number of message headers (empty message buffers) need to be allocated.
...
**Explanation:**  
A 'Clear Name' service call has been issued, but the referenced port name is statically declared.
...
**Explanation:**  
A send letter service, e.g., 'Send Letter' or 'Send Letter and Kick', has been issued which requires that the destination system name (parameter 2) is known by the local system, but the specified name is not defined as a name of a (remote) system.

---

| SEC Symbol | SEC Value | XMSG Symbol | XMSG Value |
|------------|-----------|-------------|------------|
| XMXRMFL    | 16994     | XRRMFL      | 34         |

**Description:**  
Remote system message table space full.

**Explanation:**  
A send letter service, e.g., 'Send Letter' or 'Send Letter and Kick', has been issued. The error is returned if the receiving task tries to reserve a message descriptor and a message buffer for the incoming message (letter), when there is either no free message descriptor, or no free space in the message buffer pool large enough for the incoming message.
...
| SEC Symbol | SEC Value | XMSG Symbol | XMSG Value |
|------------|-----------|-------------|------------|
| XMXRROV    | 16995     | XRRROV      | 35         |

**Description:**  
Remote task message space used up.

**Explanation:**  
A send letter, e.g., 'Send Letter' or 'Send Letter and Kick', service has been issued. The letter was sent to another task, but the receiving task was not allowed any more memory, so the letter (message) was returned to the sending task.
...
**Explanation:**  
A system routing definition service call has been issued. When the system routing is defined, XRROUT must allocate space (for the new system) in the system routing table. The size of the system routing table (i.e., the number of other systems that the local system can communicate with) is defined when XMSG is generated. When the routing table is full, this error is returned.
...
**Explanation:**  
A 'Remote Append Batch' request has been sent to the File Transfer server, but no remote batch device has been defined. In order to include the batch definition, modify the XMSG system definition file and generate a new system.
...
**Explanation:**  
A 'Send Letter and Kick' service call has been issued which requires that the specified RT program (parameter 3) exists, but the specified name is not a name of a RT-program.
...
**Explanation:**  
A 'Start Network Server' service call has been issued which requires that a network server (RT-program) is running. If the specified magic number (parameter 1) is invalid, or if the server is not running (e.g., if the server has been aborted), this error is returned.
...
| SEC Symbol | SEC Value | Description                                        |
|------------|-----------|----------------------------------------------------|
| TLERNTIN   | 17192     | The user has not done TLPINIT.                     |
| TLERBDRF   | 17193     | The user gave an invalid TLIB reference number.    |
| TLERBDCL   | 17194     | The call is not valid in the current state.        |
| TLERBDFM   | 17195     | Parameter of invalid format.                       |
| TLERBDLN   | 17196     | Parameter of invalid length.                       |
| TLERBDNM   | 17197     | Invalid name specified.                            |
| TLERBDPM   | 17198     | Other parameter error.                             |
...
| SEC Symbol | SEC Value | Description |
|------------|-----------|-------------|
| TLERNOUB   | 17199     | TLIB lacks user buffer. |
| TLERSMUB   | 17200     | The returned buffer is too small. |
| TLERDCPN   | 17201     | There is a disconnect pending. |
| TLERXOUT   | 17202     | Still expedited data is outstanding. |
| TLERNOTB   | 17203     | An attempt was made to open too many connections. Increase TLMXconnections. |
| TLERNOSD   | 17204     | An attempt was made to use too many suffixes. Increase TLMXaccessPoints. |
| TLERNOBD   | 17205     | An attempt was made to use too many user buffers. Increase TLMXbuffers. |
...
After the response came from SERVER, CLIENT could have obtained SERVER's magic number and sent messages directly. If SERVER and CLIENT were supposed to keep the communication going, XROUT would no longer be needed.
...
%
0 =: FLAGS
XMPOUT(FLAGS,MSGIDENT,PORTNUMBER) =: RETURNSTATUS
IF RETURNSTATUS )< XMOX THEN
   OUTPUT(1,'A','Bad status from XMPOUT ')
   TERMINATE(RETURNSTATUS)
ENDIF
%
```
...
2**KFWFTF =: FLAGS
XMPMFRCV(FLAGS,PORTNUMBER,MSGTYPE,REMOTEPORT,MSGIDENT,&
         LENGTHORSTAT) =: RETURNSTATUS
IF RETURNSTATUS )< XMOK THEN
  OUTPUT(1,'A','Bad status from XMPHFRCV ')
  TERMINATE(RETURNSTATUS)
ENDIF

0 =: FLAGS
XMPMFREA(FLAGS,BUFFOFFSET,ADDR(INBUFFER[0]) FORCE XMUSERADDRESS,&
         MESSOFFSET,MESSLENGTH,READLENGTH) =: RETURNSTATUS
IF RETURNSTATUS )< XMOK THEN
  OUTPUT(1,'A','Bad status from XMPMFREA ')
  TERMINATE(RETURNSTATUS)
ENDIF
OUTPUT(1,'A',INBUFFER)
%**************************************************************************
...
%
    ...
    INTEGER: SIZEBUFFER,WRITTENLENGTH,MSCTYPE,REMOTEPORT
    INTEGER: LENGTHHOSTSTAT,LENGTH,READLENGTH
    INTEGER: PORTLENGTH
...
******************************************************************************
2**XFVFF =: FLAGS
XMPRCV(FLAGS, PORTNUMBER, MSGTYPE, REMOTEPORT, MSGIDENT, &
   LENGTHORSTAT) =: RETURNSTATUS
IF RETURNSTATUS >< XMOK THEN
   OUTPUT(1, 'A', 'Bad status from XMPRCV. &
...
******************************************************************************
XMPFWRI(FLAGS, BUFFOFFSET, ADDR(OUTBUFFER(0)) FORCE XMSUSERADDRESS, &
   MSGOFFSET, SIZE OUTBUFFER, WRITTENLENGTH) =: RETURNSTATUS
IF RETURNSTATUS >< XMOK THEN
   OUTPUT(1, 'A', 'Bad status from XMPFWRI ')
   TERMINATE(RETURNSTATUS)
ENDIF
...
% 
    0 =: FLAGS
    XMPSFND(FLAGS, PORTNUMBER, REMOTEMAGIC) =: RETURNSTATUS
    IF RETURNSTATUS › XMOX THEN
        OUTPUT(1, 'A', 'Bad status from XMPSFND ')
        TERMINATE(RETURNSTATUS)
    ENDIF
%
**********************************************************************
...
I'm unable to process this image. Could you provide another image with the text?
...
4. A waiting state is entered, assuming that SERVER will respond (XMFFRCV with XFWFTP option). The response arrives at the port and resides there in an XMSG buffer. It is transferred to an internal task buffer (XMFFREA), and then output on the terminal (see the heading "Wait for message from SERVER, take it when it comes").
...
1. A port is opened and given the name "s-port". This is done by the call MFOPENM, which, in addition to opening a port, also contacts XROUT, so that XROUT's name table can be updated with the port name (see the heading "Get necessary resources from XMSG").

2. A waiting state is entered, assuming that XROUT will forward a letter from client (XMFTFRCV with XFWTIF option). The response arrives at the port in an XMSG buffer (see the heading "Wait for message from XROUT").
...
4. The same message buffer that was received from CLIENT via XROUT is being used for the message "Hello CLIENT" (XMFWFRT), and sent to CLIENT using the XMFFSND call (See the heading "Send message to client").

5. All resources are given back to XMSG.

# 1.4 Notes

SERVER does not need to obtain a message buffer from XMSG by using XMFTGET, because it can use the buffer that CLIENT sent via XROUT. SERVER does not need to read the letter from XROUT, unless it is supposed to check any user data beyond the letter header (the latter is not done in the example).
...
C
      ...
      EXTERNAL  XMFBLT,XMFOPNM,XMFGET,XMFWRI,XMFROUT,XMFRCV,XMFMST
     +,XMFSND,XMFFEA,XMFDFCT
      INTEGER  XMFBLT,XMFOPNM,XMFGET,XMFWRI,XMFROUT,XMFRCV,XMFMST
     +,XMFSND,XMFFEA,XMFDFCT

C

      INTEGER  SERIALNUMBER,RETURNSTATUS,OFFSET,MESSLENGTH,PORTNUMBER.
     +SIZEBUFFER,MSGIDENT,WRITELENGTH,MSCTYPE,REMOTEPORT,
     +LENGTHOFSTAT,LENGTH,READLENGTH,FLAGS,BUFFEROFFSET,MESSOFFSET
...
C      EQUIVALENCE (INBUF,INPUTBUFFER(0))
...
C******************************************************************
C
C SEND THE LETTER TO SERVER THROUGH XROUT
C
C******************************************************************
FLAGS = 0
MESSLENGTH = OFFSET
BUFFOFFSET = 0
MESSOFFSET = 0
RETURNSTATUS = XMWFWRI(FLAGS, BUFFOFFSET, OUTPUTBUFFER, MESSOFFSET,
+                      MESSLENGTH, WRITTENLENGTH)
IF (RETURNSTATUS .NE. XMK0) THEN
    WRITE(1,*)'BAD STATUS FROM XMWFWRI WHEN CONTACTING XROUT'
    GO TO 888
ENDIF
```
...
C******************************************************************
C
C WAIT FOR MESSAGE FROM SERVER, TAKE IT WHEN IT COMES
C
C******************************************************************
FLAGS = 2**XFWFT
...
```
                ...
                $LIST ON
C
                EXTERNAL XMFOPNM, XMFFRCV, XMFFMST, XMFFREA, XMFFWR1, XMFFSND, XMFFDCT
                INTEGER  XMFOPNM, XMFFRCV, XMFFMST, XMFFREA, XMFFWR1, XMFFSND, XMFFDCT
C
                INTEGER  SERIALNUMBER, RETURNSTATUS, MESSLENGTH, PORTNUMBER,
               +SIZEBUFFER, MSGIDENT, WRITENLENGTH, MSGTYPE, REMOTEPORT,
               +LENGTHORSTAT, LENGTH, READLENGTH, FLAGS, BUFOFFSET, MESSOFFSET
C
...
****************************************************************************
C
C GET MAGIC NUMBER OF CLIENT
C
****************************************************************************

```
FLAGS = 0
RETURNSTATUS=XMFMFST(FLAGS,MSGIDENT,MSGTYPE,REMOTEMAGIC,LENGTH)
IF (RETURNSTATUS .NE. XMOK) THEN
    WRITE(1,*)'BAD STATUS FROM XMFMFST'
    GO TO 888
ENDIF
```
...
IF (RETURNSTATUS .NE. XMOK) THEN
    WRITE(1,*)'BAD STATUS FROM XMFSND'
    GO TO 888
ENDIF
GO TO 999
```
...
**Document Number:** ND-60.164.3 EN
...
flowchart TD
    ...
    B -->|EVRQIN| D[GTRO]
    B -->|EVDCIN| E[DCIN]
    B -->|Other| F[Unexpected]
    C --> G[CNRS]
    D --> H[SNRS]
```

_Fig. 1. Flowchart of RR-SERVER_
...
flowchart TD
    A[INIT] --> B[CNRQ]
    B --> C{Do while NOT done}
    C --> D[WAIT]
    D -->|EVCNCF| E[CNCF]
    D -->|EVRSIN| F[GTRS]
    D -->|EVDCDF| G[Disconnect complete]
    D -->|Other| H[Unexpected event]
    F --> I{Done?}
    I -->|No| J[SNRQ]
    I -->|Yes| K[DCRQ]
    K --> L[END]
...
$MACRO CHKstatus(status)            % macro for status checking
  IF "status" > ' OK THEN           % in this simple example all
    outputl('.A',' $BAD status')    % errors only cause a
...
BYTES ARRAY: buffers(1:1.0:100)               % user buffer

BYTES: serverName := 'RRSERVER'    % server port name
RRID: clientID                     % used to specify remote client

BYTES: serverInfo := 'Welcome to server RRSERVER' % server user data to be sent
BYTES: response := 'this is the response'        % with connect-response and
                                                   % send-response
...
32767 =: waitTime.RRTMlength        % specification of time-out
4 =: waitTime.RRTMunits             % values
...
RRPBinit(staticRRarea,dynamicRRarea,&       % start use RR-LIB in server mode
         RRMAasServer(),serverName.&        % (RRPBinit must always be the first 
...
DO WHILE TRUE
  ...
  CHKstatus(RRstatus)
    ...
    IF nextEvent.RREvent = RREVenin THEN % connection request has arrived
      outtext('$SERVER: connect atteapt')
      nextEvent.RREVremote =: clientID % specify the remote client
      RRPSin(staticRRarea,clientID,&   % get information sent by client
      ...
      RRPSsns(staticRRarea,clientID,IND{outData}) % accept connection
                               =: RRstatus
    ELSIF nextEvent.RREvent = RREVrqin THEN % request has arrived
      ...
      RRPSsns(staticRRarea,clientID,& % send response
...
```
    ELSIF nextEvent.RREvent = RREvdcin THEN  % disconnected from client
...
BYTES: destSystem := 'DONALD'              % remote system name
BYTES: destServer := 'RRSERVER'            % remote server port name
...
BYTES: disconnectInfo := 'goodbye'              % requests
...
RRPcnrg(staticRRarea,destsystem,destServer,& % attempt to open a connection
...
DO WHILE NOT done
    RRPBwait(staticRRarea,waitTime,& % wait for an event
        requestedEvent,nextEvent) =: RRstatus
    CHKstatus(RRstatus)

    IF nextEvent.RREVevent = RREVcnf THEN % connection accepted
        outtext('$CLIENT: connect confirmation event')
        RRPcncef(staticRRarea,& % get information sent by server
            serverID,reply) =: RRstatus % when connection was accepted
        CHKstatus(RRstatus)
        outtext(' with server info')
        outtext('$')
        outtext(INd[reply])
        RRPcnrq(staticRRarea,serverID,request,& % send request
...
```plaintext
    ELSIF nextEvent.RRREvevent = RREVdecf THEN  % user-initiated disconnect
...
DO WHILE (requestsCompleted < numberRequests)
    ...
    RRPBcall(staticRRArea,serverID,request,& % send request and wait for
...
RRRPBclise(staticRRArea,serverID,&  % terminate connection to server
...
INTEGER ARRAY: TIdynamic(0:TLSZDynamic-1)  % storage for TLIB
<"""TLS: serverName := 'SERVER'           % server port name
...
# COSMOS PROGRAMMER GUIDE
## SAMPLE PROGRAMS USING TLIB/PLAN C
...
| INTEGER        | disconnectReason| % reason for disconnect                          |
| INTEGER        | credit          | % initial credit for sending data               |
| INTEGER        | status          | % status from TLIB calls                        |
...
### INISTACK stack
```
TLXMniUserMode =: mode.TLMDnxsmgmode   % initialize TLIB
                                  % XMSG in user mode
TRUE =: mode.TLMUniqueSuffix      % only ONE server active
TLMXconnections =: mode.TLMDnxConnections % max. simultaneous connections
TLMXAccessPoints =: mode.TLMNuxAccessPoints% max. no of TLAPs (ports)
TLMXbuffers =: mode.TLMDnxBuffers % max. buffers owned by TLIB
TLMXQueuedTpdus =: mode.TLMDnxQueuedTpdus% max. Q'd TPDU's in XMSG space

TLIPINIT(mode.TLIdynamic,status) % Always first TLIB call
...
O =: serverAddress.TIAPnetAddress.TLNtmpLength% don't specify own system
SIZE serverName =: serverAddress.TLASuffix.TLNLength % server name
serverName =: serverAddress.TLASuffix.TLNmstring(O:SIZE serverName-1)

TLIPTSIS(serverAddress,myReference,status,TLIbReference)
CHKstatus('TLIPtsis',status)
output1('\A', 'Server is listening - name is ')
output1('\A', serverName)

TLinfiniteTime =: maxwaitTime.TLIMTLENGTH     % wait forever for next event
TLMhrs =: maxwaitTime.TLIMUNITS

TLanyRefno =: requestedEvent.TLEVrefno       % Allow any event to occur 
TLanyEvent =: requestedEvent.TLEVcode        % on any connection
```
...
DO FOREVER
    ...
    IF actualEvent.TLEVcode = TLEVconn THEN
        ...
        IF (actualEvent.TLEVdata[0] : lengthData) > 0 THEN % if user data
            TLlogicalAddress := buffer.TLBFformat % sent by client
            ADDR(inputBuffer[0]) FORCE INTEGER := buffer.TLBAddress % then
            TLPprbf(TLIBreference,buffer,lengthData,status) % provide buffer
        ...
        IF lengthData > 0 THEN % if user data sent
            ...
            output(1,'A','''')
        ENDIF
    ELSEIF actualEvent.TLEVcode = TLEVedin THEN
        output(1,'A','%disconnect reason')
        IF (actualEvent.TLEVdata[0] : lengthData) > 0 THEN % if user data
            TLlogicalAddress := buffer.TLBFformat % then provide
        ...
        ENDIF
        TLPedin(TLIBreference,status,disconnectReason,& % get information
...
$MACRO CHK(status(text,status) % define a macro for status checking
    IF status )< OK THEN      % in this simple example, all
...
BYTES: greetings := 'greetings from client' % data to go with connect request
...
TLMDmxConnections =: mode.TLMDmxConnections     % Max simultaneous connections
TLXMaxAccessPoints =: mode.TLMDmxAccessPoints   % Max no. of TLAPs (ports)
TLMDxBuffers =: mode.TLMDmxBuffers              % Max owned by TLIB
TLMDmxQueuedTpdu =: mode.TLMDmxQueuedTpdus      % Max q'd TPDUs in XMSG space

TLIPINIT(mode, TLdynamic, status)               % Always first TLIB call
CHKstatus('TLIPINIT', status)

TLlqap2string =: myAddress.TLAPformat            % connect to server
                                                    % set up own address
0 =: myAddress.TLAneAddress.TLNlength             % do not specify own system
SIZE ownName =: myAddress.TLAPsuffix.TLNMLength   % own port name
ownName =: myAddress.TLAPsuffix.TLNMLString(0:SIZE ownName-1)

tLtlap2string =: serverAddress.TLAPformat         % set up server address
                                                      % address specified as string
SIZE serverSsystem =: serverAddress.TLAneAddress.TLNMLength
serverSystem =: serverAddress.TLAneAddress.TLNMLstring(0:SIZE serverSystem-1)
SIZE serverName =: serverAddress.TLAPsuffix.TLNLength
serverName =: serverAddress.TLAPsuffix.TLNMLString(0:SIZE serverName-1)

TLlogicalAddress =: buffer.TLBufferattr           % buffer address is logical
ADDR(greetings(0)) FORCE INTEGER =: buffer.TLBAddress, % address in user
TRUE =: quality.TLQSisDefault                     % space

1 =: myReference                                 % only one connection at a
                                                  % time, not used anyway!

output(1, 'A', '$connecting to server...')
TLIPCNQ(myAddress, serverAddress, myReference, quality, buffer, &
   SIZE greetings, status, TLIBReference)
CHKstatus('TLIPcnq', status)

2 =: maxWaitTime.TLNMLength                       % set up to wait for events
                                                  % max wait time of 2 minutes
TLTMnins =: maxWaitTime.TLTMnunits
...
Scanned by Jonny Oddene for Sintran Dag @ 2021
...
DO WHILE NOT done
    ...
    IF actualEvent.TLEVcode = TLEVcncf THEN
        ...
        IF (actualEvent.TLEvdata(0) ::= lengthData) > 0 THEN % if user sent
            TLogicalAddress ::= buffer.TLBJFormat % data
            ADDR(inputBuffer(0)) FORCE INTEGER ::= buffer.TLBADDRESS
        ...
        IF lengthdata > 0 THEN           % display user data
            ...
            output(1,'A','"’)
        ENDIF
    ELSEIF actualEvent.TLEvcode = TLEVdecf THEN % disconnection
        output(1,'A','disconnect complete')     % complete
        TRUE ::= done
    ELSE
        output(1,'A','$UNEXPECTED EVENT type')
        output(1,'16',actualEvent.TLEvcode)
        TRUE ::= done
    ENDIF
ENDDO
...
I'm unable to extract any technical content from the page since it appears to be mostly blank, aside from the visible text at the top and bottom:

```
396                                           COSMOS PROGRAMMER GUIDE
...
C  declarations
     EXTERNAL TLFINIT,TLFCNRQ,TLFAWIT,TLFNCF,TLFDCIN
...
C  storage for return status from TLIB calls
     INTEGER RETSTAT
C  TLAP definitions
     INTEGER*4 USADD(0:TLATLLAP-1)
     INTEGER*4 IMADD(0:TLATLLAP-1)
...
C  and require the T-suffix names to be unique
     MODE(TLMODUNIQUESUFFIX) = TLISUNIQUE

C
     MODE(TLMXMCONNECTIONS) = TLMXCONNECTIONS
     MODE(TLMXMXACCESSPOINTS) = TLMXACCESSPOINTS
     MODE(TLMXMXBUFFERS) = TLMXMBUFFERS
     MODE(TLMXMQUEUEIDTPDU'S) = TLMXQUEUEDTPDUS
...
C timeout definition
    MAXWAIT(TLTMINUTS)=TLTMINS
    MAXWAIT(TLTMLENGTH)=10

C requested event definition
    REQUEST(TLEVERFENO)=TLANYREFNO
...
CALL TLFPBF(ITLREF,USERDATA,LDATA,0,RETSTAT)
IF(RETSTAT.NE.OK) THEN
    WRITE(1,*)'BAD STATUS FROM TLPBF. DISCONNECT DATA'
    GO TO 999
ENDIF
...
C define array to store TLIB mode  
```
INTEGER MODE(0:TLLAMODE-1)
```
C define array for timeout period on wait  
```
INTEGER MAXWAIT(0:TLLTIME-1)
```
C define arrays to store requested and actual events  
```
INTEGER REQUEST(0:TLLAEVENT-1)
INTEGER ACTUAL(0:TLLAEVENT-1)
```
C define array to store user data on connection open and close  
```
INTEGER USERDATA(0:(TLLXMUSERDATA/2)-1)
```
C storage for return status from TLIB calls  
```
INTEGER RETSTAT
...
C TLAP definitions  
```
INTEGER*4 REMADD(0:TLLTLAP-1)
INTEGER*4 USADD(0:TLLTLAP-1)
```
C network address and suffix definitions  
```
CHARACTER RN*(TLLXMNAMELENGTH),RS*(TLLXMNAMELENGTH),
          +UN*(TLLXMNAMELENGTH),US*(TLLXMNAMELENGTH)
EQUIVALENCE (REMADD(TLAPNETADDRESS),RN),(REMADD(TLAPSUFFIX),RS)
EQUIVALENCE (USADD(TLAPNETADDRESS),UN),(USADD(TLAPSUFFIX),US)
CHARACTER NETNAME*(TLLXMNAMELENGTH),SUFFIXNAME*(TLLXMNAMELENGTH)
CHARACTER NETNAM1*(TLLXMNAMELENGTH),SUFFIXNA1*(TLLXMNAMELENGTH)
```
...
C quality of service  
```
INTEGER QOS(0:TLLAQOS-1)
```
C initialize TLIB:
C XMSG is called in user mode  
```
MODE(TLLMDXMGSMODE) = TLLXMIUSERMODE
```
C and require the T-suffix names to be unique  
```
MODE(TLLMUNIQUESUFFIX) = TLISUNIQUE
```
C  
```
MODE(TLLMDXCONNECTIONS) = TLLXMCONNECTIONS
MODE(TLLMXACCESSPOINTS) = TLLXMACCESSPOINTS
MODE(TLLMDXBUFFERS) = TLLXMUBUFFERS
```
...
```
MODE(TLIMXQUEUEDPDPDUS) = ILMXQUEUEDPDPDUS
CALL TLFINIT(MODE,TLDYNAMIC,RETSTAT)
...
C timeout definition
MAXWAIT(TLTMUNITS)=TLTLMINNS
MAXWAIT(TLTMLENGTH)=10

C requested event definition
REQUEST(TLEVREFNO)=TLANYREFNO
REQUEST(TLEVOCODE)=TLANYEVENT

C send connection request
MESS$='HELLO '
IUSREF=1
LENDAT=6
QOS(TLQOSISDEFAULT)=TLISDEFAULT
CALL TLFCNRQ(USADD,REMADD1,IUSREF,QOS,USERDATA,LENDAT,RETSTAT,+TLIRET)
IF(RETSTAT.NE.0)THEN
...
CALL TLFIWAIT(MAXWAIT,REQUEST,RETSTAT,ACTUAL)
IF(RETSTAT.NE.0) THEN
    WRITE(1,*)'BAD STATUS FROM TLFIWAIT'
    GO TO 999
ENDIF
IF(ACTUAL(TLEVOCODE).EQ.TLEVTIME) THEN
    WRITE(1,*) 'TIME OUT'
    GO TO 999
ENDIF
IF(ACTUAL(TLEVOCODE).NE.TLEVCNCF) THEN
    WRITE(1,*)'UNEXPECTED EVENT CODE',ACTUAL(TLEVOCODE)
    IF(ACTUAL(TLEVOCODE).EQ.TLEVDCIN) THEN
        CALL TLFICN1(TLIRET,IFSTAT,IDCRSN,IUBFID,LENDAT)
...
- buffers in TLIB .............................. 208.
- bufferSize ................................... 38.
- bytes0To1 .................................... 57, 62, 72, 85, 131, 146, 159.
- bytes0To1orStat .............................. 60, 134.
- bytes2To3 .................................... 57, 72, 85, 131, 146, 159.
- bytes4To5 .................................... 57, 72, 85, 131, 146, 159.
...
### localPort
50, 52, 58, 60, 62, 68, 82, 84, 125, 127, 132, 134, 136, 142, 156, 158
...
| Topic                               | Pages                   |
|-------------------------------------|-------------------------|
| phases during communication         | 203.                    |
| PIOC                                | 3.                      |
| port                                | 4.                      |
| allocation                          | 4.                      |
| default                             | 4.                      |
...
| name                                | 4.                      |
| number                              | 4.                      |
| portName                            | 19, 26, 77, 79, 95, 102, 151, 153. |
| portNo                              | 44, 119, 138.           |
| portNoFound                         | 44, 119.                |
| portNumber                          | 26, 31, 42, 45, 47, 48, 64, 75, 77, 79, 102, 107, 117, 120, 122, 123, 149, 151, 153. |
| privileged tasks                    | 6.                      |
| quality                             | 218-221.                |
| queue message                       | 4.                      |
| queueLength                         | 50, 125.                |
| readLength                          | 28, 54, 81, 104, 129, 155. |
| reason                              | 172, 184, 186, 190.     |
| registerBlock                       | 34, 109.                |
| remote address                      | 219, 220.               |
| remoteID                            | 171-173, 178, 179, 181, 182, 184, 186, 188-191, 195, 196, 198, 199. |
| remoteMagicNum                      | 46, 68, 84, 121, 142, 158. |
| remotePort                          | 50, 52, 58, 60, 125, 127, 132, 134. |
| request                             | 181, 184, 199.          |
...
| restartAddress                      | 71, 145.                |
| retstat                             | 218-228, 230-233.       |
| return status                       | 7.                      |
| routine types                       | 6.                      |
| RR-LIB prefixes                     | 163.                    |
...
| RRDCrmcq (Error symbol) | 342.                  |
| RRDCrmdd (Error symbol) | 343.                  |
| RRDCunsr (Error symbol) | 342.                  |
| RRDCunsy (Error symbol) | 342.                  |
| RRDCuser (Error symbol) | 342.                  |
| RRDCxsin (Error symbol) | 343.                  |
| RRERbdbe (Error symbol) | 339.                  |
...
| RRERtslm (Error symbol) | 340.                  |
| RRERunew (Error symbol) | 338.                  |
...
### userLength 
- 54, 58, 73, 81, 86, 129, 132, 147, 155, 160.

### writtenLength
- 73, 86, 147, 160.

| Error Symbol | Page Number |
|--------------|-------------|
| XEAIN        | 316         |
| XEBFC        | 316         |
| XEBNC        | 317         |
...
| XMTNO       | 8    |
| XMXEAIN (Error symbol) | 316 |
| XMXEBFC (Error symbol) | 316 |
...
|-----------|-----------------|------|
| XMXRMMP   | {Error symbol}  | 324  |
| XMXRMTL   | {Error symbol}  | 325  |
| XMXRNCO   | {Error symbol}  | 334  |
| XMXRNEI   | {Error symbol}  | 326  |
| XMXRNGA   | {Error symbol}  | 333  |
| XMXRNLS   | {Error symbol}  | 329  |
| XMXRNNA   | {Error symbol}  | 333  |
| XMXRNNE   | {Error symbol}  | 332  |
| XMXRNRO   | {Error symbol}  | 326  |
| XMXRNSE   | {Error symbol}  | 330  |
| XMXRNSP   | {Error symbol}  | 324  |
| XMXRNTR   | {Error symbol}  | 328  |
| X::RNXLD  | {Error symbol}  | 327  |
| XMXRNXL   | {Error symbol}  | 327  |
| XMXRNXM   | {Error symbol}  | 326  |
| XMXRPVV   | {Error symbol}  | 325  |
| XMXRRFU   | {Error symbol}  | 332  |
| XMXRRNA   | {Error symbol}  | 330  |
| XMXRRND   | {Error symbol}  | 333  |
| XMXRRNL   | {Error symbol}  | 334  |
| XMXRROV   | {Error symbol}  | 331  |
| XMXRRPN   | {Error symbol}  | 331  |
| XMXRSMF   | {Error symbol}  | 325  |
| XMXRSNR   | {Error symbol}  | 332  |
| XMXRSOK   | {Error symbol}  | 323  |
| XMXRSYD   | {Error symbol}  | 329  |
| XMXRTFE   | {Error symbol}  | 328  |
| XMXRTI5   | {Error symbol}  | 329  |
| XMXRTRA   | {Error symbol}  | 328  |
| XMXRTRE   | {Error symbol}  | 330  |
| XMXRTRP   | {Error symbol}  | 328  |
| XMXTRRT   | {Error symbol}  | 329  |
| XMXRUK5   | {Error symbol}  | 331  |
| XMXRUNM   | {Error symbol}  | 324  |
| XMXRUNN   | {Error symbol}  | 323  |
| XMXRURT   | {Error symbol}  | 332  |
| XRAMB     | {Error symbol}  | 334  |
| XRBLK     | {Error symbol}  | 329  |
| XRBUS     | {Error symbol}  | 330  |
| XRDDDF    | {Error symbol}  | 323  |
| Xreg      |                 | 66, 140 |
| XRFFU     | {Error symbol}  | 334  |
| XRJ1IV    | {Error symbol}  | 326  |
| XR1LN     | {Error symbol}  | 327  |
| XR1IPT    | {Error symbol}  | 324  |
| XR1PQ     | {Error symbol}  | 334  |
| XRIQE     | {Error symbol}  | 333  |
| XRISE     | {Error symbol}  | 333  |
| XRISN     | {Error symbol}  | 323  |
| XRISY     | {Error symbol}  | 325  |
| XRRMFL    | {Error symbol}  | 331  |
| XRMMMP    | {Error symbol}  | 324  |
| XRMRTL    | {Error symbol}  | 325  |
...
| XRSYD (Error symbol)     | 329  |
| XRTPF (Error symbol)     | 328  |
| XRTI5 (Error symbol)     | 329  |
| XTRRA (Error symbol)     | 328  |
| XTRRE (Error symbol)     | 330  |
| XTRRP (Error symbol)     | 328  |
| XRTRT (Error symbol)     | 329  |
| XRUKS (Error symbol)     | 331  |
| XRUMN (Error symbol)     | 324  |
| XRUNN (Error symbol)     | 323  |
| XRURT (Error symbol)     | 332  |
| XSCNM                    | 293  |
| XSCRS                    | 290  |
| XSDAT                    | 304  |
| XSDLO                    | 294  |
...
I'm sorry, but the image you provided appears to be blank or doesn't contain visible text or diagrams that can be converted to Markdown. If there's any other content you'd like me to assist with, please let me know!
...
Your name: __________________________________________ Date: __________

Company: ________________________________________ Position: __________

Address: ___________________________________________

What are you using this manual for? ___________________________________
...
Answered by ________________________________ Date _______________
```
...
I'm sorry, I can't process the image properly. Could you please provide text or a different form of the content?
