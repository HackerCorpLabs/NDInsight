...
Looks like the provided image is either blank or doesn't contain meaningful content to convert to Markdown. If you have another page or document, please provide that for conversion.
...
This manual describes DOMINO and NUCLEUS. Most of the modules are documented in separate chapters.

## The reader

This manual is intended for maintenance personnel and system developers.

## The Products

The OS-kit consists of several software modules to be used for running and developing system software on the DOMINO IO-controllers. DOMINO is only available on ND-5000 computers with MF-Bus memory (former MPM-5), whereas NUCLEUS is also available on ND-500-II with OCTOBUS. The DOMINO controllers are based on the Motorola MC-68020 microprocessors.
...
The user should be familiar with general program development on ND computers. It is not necessary to know much about the Motorola assembly language as most of the programs can be written in PLANC.

---

## Page 6

## Important Objects for Program Development
...
| OPEN-PATH                 | 46   |
| CHANGE-PATH               | 48   |
| TEST-COMMUNICATION        | 50   |
| USE-MAILBOX               | 52   |
| LIST-MAILBOX-PARAMETERS   | 52   |
| TRANSPARENT-MODE          | 53   |
| SET-BREAK-CHARACTER       | 54   |
| LIST-BREAK-CHARACTER      | 54   |
| SET-DOPCOM-PARAMETERS     | 55   |
| LIST-DOPCOM-PARAMETERS    | 55   |
| SET-MICE-PARAMETERS       | 56   |
| LIST-MICE-PARAMETERS      | 57   |
...
8. PROMA N(Processor Manager) error codes ....... 263  
...
```mermaid
flowchart TB
    A[Inside host computer]
    B[OCTOBUS controller / MFbus adapter]
    C[MC-68020<br>Local RAM<br>OPCOM PROM<br>Device PROM<br>Memory protect<br>USART / timers]
    D[Case (device)<br>dependent hardware<br>(storage media,<br>terminals or data<br>communication)]
    E[Terminal and<br>trace interface]
    F[Device interface]
    G[Operator<br>terminal]
    H[MICE / logic<br>analyzer]
    I[To device]
...
The DOMINO Monitor is an ordinary SINTRAN user-program, which is used for downloading and debugging of applications in the DOMINO controllers.
...
```mermaid
flowchart TD
    USER --> |ND-500| A[DOMINO Monitor\nSuper kernel]
    A --> |XMSG| B[BOPCOM server\n(RT-program)]
    B --> |MPIT| C[DOMINO OPCOM]
    B --> |ND-100| B
    B --> |Ring 2| B
    C --> |OCTOBUS| D
...
The page has no visible text or diagrams to transcribe into Markdown.
...
- **<product id>**  
  Standard part to identify product relationship, the product here being the Processor Manager-server, PROMAN, which has the product prefix **PMA**.

- **<hardware id>**  
  Identifier for h/w module number (4 chars). This identifier is looked up by the PROMAN program as a function of the hardware module number, see table 1 on page 14.

- **<basic software id>**  
  Software type identifier (max 4 chars). Necessary when more than one type of product runs on processors with the same module number. Examples are TCP/IP, COSMOS and SIBAS-communication, all running on Ethernet-III. This field is NOT intended to take care of version control.
...
```
  ...
  |_______|           |                   |
    ...
    |                 |                   |
   \|/               \|/                 \|/
Standard part   Defined at           Given by
for images      configuration        Module Number
                (Basic Software Id)  (see table)
                   |
                   |
                  \|/
...
└───────────────────────────┘
 ...
 │  Default user is RT, and default file type is :IMAG.  │
...
Each DOMINO controller has at least three LEDs (light emitting diodes) in three different colors:
...
|  This means that the selftest |
|  after reset has found some   |
|  fault. The controller may be |
|  used if the hardware can be  |
|  avoided. (For instance the   |
|  protection system).          |
---------------------------------
```
...
Regular flashing long OFF/short ON: Something unusable received from OCTOBUS.
...
Three short ON/one long OFF: MPROTSET returns error when initially setting up memory protection.

    █ █ █ █ █ █ █ █ █ █ █ █ █
    █   █ █                              

One long ON/two short ON: The host switch stack is empty; nobody to send to.

    █ █ █       █ █ █  
    █       █             

Three short ON/one long ON: OCTOBUS driver interface called with invalid function code.

    █ █ █  █     █   █  
    █                              

Four short ON/one long ON: OCTOBUS driver returns error when connecting to emergency message 177B or 176B.

    █ █ █ █    █ █  █
    █                            

Seven short ON/one long ON: The path to be used is unknown in OPCOM.
...
The preboot tests run in EPROM, and verify all necessary hardware to be able to boot and run in DRAM (DOMINO Local Memory). The postboot tests consist of two parts:

- Standard postboot tests
- Device dependent postboot tests

The 'Standard postboot tests' is executed on all DOMINO controllers and test all the standard hardware parts such as MFP, interrupt system, protect system, BADAP, OBCON, etc. The 'Device dependent postboot tests' are tests specific for each type of DOMINO controllers, and test all the special hardware functions of the different controllers.

All standard tests (Preboot tests and Standard postboot tests) are located in the DOMINO OPCOM prom, while the device tests are located in the device prom.
...
```mermaid
flowchart TD
    A[RESET] --> B["NON-Destroying (applies to memory) so-called 'preboot test'"]
    B --> C{"Power fail recovery"}
    C -->|No| D["post boot tests"]
    C -->|Yes| E["Enter 'WAITCONT' state\nWait for DOMINO OPCOM-command\nWAITCONT: see page 98"]
    D --> F["Initialize controller HW and OPCOM structures"]
    F --> G["Enter 'aborted' state"]
    G --> H["Wait for DOMINO OPCOM command"]
    E --> |(See previous section)| G
```
...
```mermaid
flowchart TD
    ERROR_DEVICE[ERROR device] --> S3WatchDog
    S3WatchDog --> PROMAN
    PROMAN[["PROMAN\nEvent msg\ngateway\nserver"]] --> OCTOBUS
    ND_100[ND-100] --> PROMAN
    OCTOBUS --> DOMINO_CONTROLLER
    PMA_dump_log[PMA-dump-log\nprogram] -->|Ring file| PMA_ERS_BUFFER
    PMA_ERS_BUFFER -->|Inspect ERS-msg| INSPECT
    DOMINO_CONTROLLER[DOMINO\ncontroller] --> OPCOM
    OPCOM --> DOMINOS
    DOMINOS -->|PIRfatal| PIRfatal
    PIRfatal --> ERS_LIB
    ERS_LIB[ERS LIB] --> DOMINO_APP
    DOMINO_APP[DOMINO\napplication\nsoftware]
...
| when the PROMAN server is   |
| active. If you wish         |
| to change the size or       |
| delete it, please do this   |
| before the server is        |
| started, or immediately     |
| before restarting the       |
| system. If the server is    |
| unable to log to            |
| the ring file, a message    |
...
Hardware Module Number tells what kind of card (type of DOMINO in this context) that is present in the slot. This is the origin for determining the hardware identifier in the image name, (see section about Automatic configuration, page 11).
...
`<image file>` = Name of image file, default filetype is :IMAG, default user is RT
...
All commands, domains and file names may be abbreviated as long as they are unambiguous. The abbreviation rules are as for SINTRAN. The full range of SINTRAN editing characters is available.

The DOMINO Monitor will prompt for missing default parameters.

## ESCape

A Command and parameter collection can be aborted by pressing ESCape. The user returns to the command level in the Monitor. Command execution may also be interrupted in this way. Special NOTIS keys generating ESCape characters are therefore also harmless to the program.

## @

If the first character of a command line is `@`, the rest of the line is taken to be a SINTRAN command. The command is checked before being sent to SINTRAN. This safeguards against starting another program unintentionally and thus causing automatic termination of the Monitor.

## &

The character `&` means that the input line is continued on the next line.
...
| Purpose |
| ------- |
| Terminates the execution of the DOMINO Monitor. Note that EXIT cannot be used within a macro. This command is used for releasing reserved resources (for example the DOMINO controller). All breakpoints - if any - are released, and if possible the control is given back to the Processor Manager. |
...
| Purpose |
| ------- |
| All commands matching `<Command>` will be written together with their parameters to the output file. |

| Command |
| ------- |
| Any command abbreviation, ambiguous or nonambiguous. Default is all commands. |

| Parameters |
| ---------- |
| Note that HELP may also be used for some parameters to obtain a list of legal choices. |
...
| Example                                              |
+------------------------------------------------------+
| DM(SERVER): (ASYL)LOOK-AT-STACK↵  %command performed |
|                        on ASYL                       |
| DM(SERVER): LIST-MICE-PARAMETERS↵ %affects only      |
|                        DOMINO-MONITOR                |
+------------------------------------------------------+
```
...
```mermaid
graph TD
    A[DOMINO monitor] -->|XMSG| B[BOPCOM server]
    B -->|OCTOBUS| C[DOMINO controller]
    C --> D[DOMINO OPCOM]
    D --> E[DOMINOS]
    E --> F[DOMINO "application" SW]
    F --> G[Device]
    B -->|RT-program in ND-100.| C
```
...
| Connected to MC68020 based controller %connection     |
|                                                    established |
| DM(SERVER): OPEN-PATH ASYL ASYL-DISC               %Opening    |
|                                                    another path|
| Connected to MC68020 based controller %ASYL        |
|                                                    becomes     |
|                                                    current     |
|                                                    %path       |
| DM(ASYL): CHANGE-PATH SERVER                       % Switch back|
|                                                    to SERVER   |
|                                                    path        |
| DM(SERVER):                                         |
+----------------------------------------------------+
...
```mermaid
flowchart TD
    A(DOMINO monitor) -->|ASYL (RS232)| B(DOMINO controller)
    B --> C(DOPCOM)
    C --> D(DOMINOS)
    D --> E(DOMINO "APPLICATION" SW)
    E --> F(Device)
```
...
Tests communication between the DOMINO Monitor and the target via the standard path. The test is performed by writing and reading several bit patterns. The communication cannot be tested via MICE.

**Number of times**

The number of times to run the communication test.
...
graph TD;
    A[DOMINO monitor] -->|XMSG| B[BOPCOM server];
    B -->|RT-program<br>in ND-100.| C;
    C -->|MFbus| D;
    D --> E(Multiport<br>Memory);
    E --> F(MAIL<br>-BOX<br>N-1);
    E --> G(MAIL<br>-BOX<br>N);
    E --> H(MAIL<br>-BOX<br>N+1);
    D -->|OCTOBUS| I;
    I --> J[DOMINO controller<br>N];
    J --> K[OPCOM];
    K --> L[DOMINOS];
    L --> M[DOMINO<br>"APPLICATION"<br>SW];
    M -->|Device| I;
```
...
| **Mem\***    | Memory timeout. The maximum time (ms) to wait after requesting a memory location a break point change from MICE, or after giving the GO command. |
...
Both old and new domain formats are supported.
...
Download a file from the SINTRAN file system into the controller.
...
This command is used when a domain is already placed in the target's memory and has been aborted during execution. The command allows investigation of the aborted domain.

**Domain**

The name of a domain in the specified description file. The `<Domain>` may be prefixed with COSMOS RFA notation:  
`System(Remote-user(Password)).(User)Domain`
...
Compose a new macro from the basic commands or other macros.
...
The name of the new macro. It can consist of any number of visible characters except space or comma.

### Macro body

Every line following the DEFINE-MACRO command is taken as the macro body until the END-MACRO is encountered. It must be written on the beginning of a new line. It can be abbreviated to just E. The macro contents will not be checked before execution.

### Parameters

It is possible to define formal parameters within the macro body. They are replaced by actual parameters when the macro is called. A parameter is defined by
...
**Purpose**  
The macro with the specified name is processed. Formal parameters are substituted with actual parameters.

**Macro name**  
The name of an existing (temporary or permanent) macro.

**Parameters**  
Actual parameters to replace the formal parameters in the macro. Each parameter must be specified on a separate line. The parameter may contain any character except space or comma.
...
- **Purpose**: The names of the macros with names matching the specified name are listed on the output file. Only temporary macros are listed. Permanent macros are listed by the SINTRAN command:
...
| **BREAK** | Set break point at one of the items routines, labels or source line numbers. A line number is given relative to the start of the program. The previous break point defined by this command (if any) is reset. An optional parameter is present. It allows for specification of either `<Count>` or `<Condition>`. `<count>`, tells how many times program control shall pass the breakpoint before execution halts. The execution halts just before performing any statements of the specified item. `(<condition>)` is for giving a Boolean expression constructed of constants, variables and the operators (`+ - < > >< * / ** =`); which must be true when the breakpoint is reached, for execution to halt. |
...
Displays records in a single-linked list (linear list). The data structure is identified by a pointer to head of the list, `<start>`, and a pointer within the record to next, `<link>`. The parameter `<count>` gives the number of records to be displayed. Type CR to display next record.

## MACRO

Erase, list or build a macro. The macro is listed if no parameters given. It is erased if no body is given as 2. parameter.

## PRINT-HISTOGRAM

List the information accumulated in the histogram on an `<Output File>`.

## RESERVE-TERMINAL

Reserve an additional, free terminal. The user communication with the Debugger switches to this terminal. Communication with the application still goes via the first terminal.

## RESET-BREAKS

Reset current breakpoints.

## RUN

Start program at specified program address. Execution continues if no address is given. The DOMINO Monitor connects to the target. The break character must be typed to return to the Debugger's command processor.

## SCOPE

Switch observation scope to specified, active module or routine. Default is the current scope. This command does not affect the program execution, only the set of variables that may be inspected.

## SET

Assign value to a variable. The value may be a constant or an expression. The variable may be simple or composite.
...
Define a program area to be logged in the histogram. The histogram gives the percentage of CPU time spent on different program parts. The program area is identified by the parameters `<Start address>` and `<Maximum address>`. The program area is divided into `<interval>` equal partitions, logged individually. The maximum is 64 intervals.
...
NOTE! LOG-LINES and LOG-CALLS and some other commands are
...
| Example                                 |
+-----------------------------------------+
| DM(SERVER):BREAK 40013B 1 LOOK-AT-DATA  | 
| 67716B                                   |
| DM(SERVER):RUN                           |
+-----------------------------------------+
...
S* NOP               % is a wait loop
Endfor

DM(SERVER): STEP↵
401016B: MOVE.Q #1,D0     % Initial value of i
401020B: EXT DO; EXT.L DO % Sign extend D0
...
**Purpose**  
Set the numeric format to be used when displaying numbers. Octal is set as main format when the DOMINO Monitor is entered.

**Format**  
OCTAL, HEXADECIMAL, or DECIMAL or abbreviation of one of these.
...
_Figure 8. PLANC-MC ordinary stack frame_

| Item     | Description                                                                                   |
|----------|-----------------------------------------------------------------------------------------------|
| STP -    | Points to the first free location of the stack. The stack grows both upwards and downwards.   |
| Unused   | Reserved word for future extension.                                                           |
| SMAX -   | Points to the top of the free stack. This is the same as the A7-register for the current stack. The variable is needed as there may be several stacks in use. The value of A7-register changes after each stack initialization (Inistack). |
...
If a routine makes error return (ErReturn), a jump is made to the PLANC runtime routine #XRET. The address of #XRET is always in the A5-register. The #XRET routine performs error return to the previous level. The current stack frame is popped on the stack. The D0-register keeps the ERRCODE value.
...
```
'<string>' ↵
The memory can be modified by an ASCII string by 
...
| % copied to the breakpoint table    |
| % inside OPCOM, before being        |
| % replaced by the BKPT instruction  |
+-------------------------------------+
```
...
Displays an item from an absolute address. Addresses are otherwise taken as relative addresses.
...
|                                      |
| process  state    prio  event buffer |
| p-counter  time used                 |
|                                      |
| PRO1      blocked  1     0      0    |
| 200                                 |
...
The DOMINO controller has a flexible memory protection system. The memory protection can be changed dynamically while running programs. The microprocessor's user and supervisor mode of operation can be given separate access rights for the same memory area. The command can be repeated to protect several areas.

When the controller starts, a default protection setting is made. This is modified if DOMINOS is loaded and started.

**Address**

The `<From address>` and `<To address>` give the memory area to be protected. Seen from hardware the local memory is divided into segments of 1024 bytes each which can be protected individually.

**Supervisor Mode**

Access rights for area when the microprocessor runs in supervisor mode.

**User Mode**

Access rights for area when the microprocessor runs in user mode.

The basic legal access rights are: Fetch, Read-Write, Read-Only, No-Access. Fetch means that the contents of the memory area can be executed as instructions. Fetch should normally not be used together with Read and Write. Two PIOC-compatible modes are supported instead.
...
**Purpose**  
Gives mainly static information about the target by displaying the following:
...
**Loop type**  
Read, Read-Compare, Write, Write-Read, Write-Read-Compare

**Data type**  
Byte, halfword or word (32 bits).

**Address 1 and 2**  
The memory addresses to be accessed as defined by Loop type

**Pattern 1 and 2**  
If the loop type includes write of data, Pattern 1 is written into Address 1, and similarly for the second pair of parameters. If the loop type includes Compare, the patterns are used as "the data expected".

---

## Page 116

I'm sorry, but the page you provided does not contain any visible text or diagrams to convert into Markdown. If you have another page or additional information, please provide it so I can assist you.
...
```mermaid
flowchart TD
    A[configuration for <controller type>] --> B[linker is '(linkage-loader)']
    A --> C[domain is <domain>]
    C --> D[with <segment>]
    D --> E[place at <address><br>(def. 400000B)]
    E --> F[buffer + <value><br>(def. 0)]
    F --> G[system] --> H[LOAD LIST]
    H --> I[USER ADDRESS PART]
    I --> J[EXTENSION PART]
    J --> K[PROCESS PART]
    K --> L[endconfig]
```
...
graph TB
    ...
    C --> D{(def. AUTO\_START)}
    A --> E[data at]
    E --> F{\<user and UDSE data address\>}
    F --> G{(def. 1000000B)}
    A --> H[iospace from]
    H --> I{\<address\>}
    I --> J{(def. 70000000B)}
...
graph TD;
    ...
    begin --> |action| up;
    begin --> |action| down;
    up --> |action| ROUTINE LIST;
    down --> |action| ROUTINE LIST;
```
...
> **NOTE!** The use of INSERT statements can result in incompatibility when larger changes in the generated mode file structure are introduced with a new version of the configurator.

```mermaid
flowchart TD
    A[insert] --> B['<any string>']
    B --> C
    C -->|Loop| A
```
...
Trapping of errors from the DOMINOS services using PLANC On Routineerror is not possible. If nothing else is mentioned, all out parameters in the record are undefined in case an error code is returned.
...
> **NOTE!** Most CPU registers (D0:D7, A0:A4) may be overwritten after the call. This is especially important in user-written interrupt handlers where all registers need to be saved. The PIOC compatible TRAP #2 sequence, where the registers are saved, can be used in interrupt handlers, or even better, the interrupt handler itself can save all registers on entry and restore them on exit.
...
![Scanned by Jonny Oddene for Sintran Data © 2011](Scanned by Jonny Oddene for Sintran Data © 2011)
...
Dormant means that the process exists but is completely passive.

**Blocked**

Blocked means that the process is in a wait state. For the time being, only "waiting for event" is a possible reason. User-defined services may create more reasons.

**Ready**

The process is in the ready queue because it is ready to execute. The process is not executing because there are other process(es) with higher or equal priority in the queue.

**Running**

That process in the set of ready processes which has the highest priority, and thus is the current executing one.
...
flowchart LR
    ...
    B --> C[Process C\nprior 20\nready]
    C --> D[Process D\nprior 17\nready]
    C -->|When time limit of A expires, A is moved after C\nand B becomes running. D keeps its relative position.| D
```

_Figure 18. Round-robin scheduling_

## 5.3.1 Create service

**Explanation**  
Create a process. This means in particular:
...
PITypePrio contains in fact several parameters:
...
|    Export Auto_Start            |
|    $Include DOMI-APPL-IE:IMPT   |
|    Import (Routine Integer,     |
|            Void : ErrCheck)     |
|    Import (Program : PRO2 )     |
|    Integer Array : S (0:1023)   |
|    Constant Prior = 13          |
|    PITCreate : CreRec :=        |
|      (0,'PRO2',PIABegin+Prior,  |
|      Addr(PRO2))                |
|    Program : Auto_Start         |
|      IniStack S                 |
|      Output(1,'A',              |
|        '$Auto_Start is creating |
|         PRO2')                  |
|      Addr (CreRec) PIRCreate    |
|        ErrCheck                 |
|    Endroutine                   |
| Endmodule                       |
|_________________________________|
...
|   IniStack S                                |
|   Output(1,'A','$PRO2 running')             |
|   Output(1,'A','$I will now make myself     |
|          dormant')                          |
|   Nil PIREnd ErrCheck % Usually never       |
|                           reached !!        |
| Endroutine                                  |
| Endmodule                                   |
|                                             |
...
|     Output(1,'A','$Creating process PRO2')     |
|     Addr (CreRec) PIRCreate ErrCheck           |
|     CreRec.PIProcess =: AboRec.PIProcess =:    |
|        BegRec.PIProcess                        |
| Do                                             |
...
```mermaid
flowchart LR
    A[Integer2] -->|IN| B[PITPrName record]
    B --> C[OUT]
    B --> D[PIPprocess]
    D --> E[PINName(0:3)]
    C --> F[Bytes]
...
Set event(s) for a process. The events given by PIEEvents are added (ORed) to the event buffer of the receiving process. There is no event queuing in case some of the events already are set for it. An event may only be sent to one process at a time (no broadcast possibility).
...
```
+----------------------------------------------------+
| MODULE MASTER %This example is only relevant when  |
|               %loss of events is acceptable, or    |
|               %there is a certain maximum delay    |
|               %before the receiver reacts.         |
...
| ProRec.PIProcess =: SetEvRec.PIProcess % ProRec of |
|                                     % slave        |
| 1 =: SetEvRec.PIEvent; 0 =: KickCount % Bit/Event  |
|                                            % 0 set |
|                                                    |
| DO                                                 |
|   ++ KickCount                                     |
...
| -1 =: WaitEvRec.PIWaitEvents % return when any     |
|                              % event               |
| DO                                                 |
...
The data structure for buffer management is a double-linked list of elements. Each element keeps information about one buffer. The elements are ordered according to increasing memory address. If a buffer is given to a process, a flag is set in the corresponding list element. When DOMINOS starts, there is one list element spanning the whole pool. As buffers are given to processes, the number of list elements increases (fragmentation). The buffer list is checked for consistency. Inconsistency during DOMINOS start leads to a fatal system error (DOMINOS is aborted).
...
- **PISubFunc**: This modifier indicates in which pool the buffer resides. Use PIUsrBuff for user pool and PISysBuff for system pool.
- **PIBuffAddr**: The address (32-bit pointer) to the start of the buffer.
...
- **PIRealTime**: An INTEGER4 variable which is incremented by the clock driver on each timer interrupt (every 5 msecs). It is initialized to 0 on DOMINO startup.
- **PIHostNumb**: The number of the host CPU as an INTEGER2 variable. For DOMINO controllers the contents of the variable is not currently defined.
- **PIControll**: The number of the controller in the current host system as an INTEGER2 variable. The contents on DOMINO is the OCTOBUS Station number of the controller.
...
**PISubFunc**  
For future extension. Must be set to 1.

**PIError**  
The user-defined error code to return. Extended to be an `INTEGER 4`, but only the lower 16 bits are used.
...
**UDSE**  
All these entities are hereafter referred to as User-Defined System Extensions (UDSE).
...
| INTEGER2 : SaveSR                             |
|                                               |
| $* MOVE.W  SR, SaveSR   % keep current value  |
|                         % of SR               |
| $* ORI.W   #0700H,SR    % set interrupt       |
|                         % threshold to        |
|                         % maximum             |
|                         % uninterruptible     |
|                         % code                |
| $* MOVE.W  SaveSR, SR   % interrupts          |
|                         % switched on again   |
|                                               |
+-----------------------------------------------+
```
...
| $* ORI.W   #0700H,SR % disable all maskable     |
|                      % interrupts               |
| $ENDMACRO                                       |
|                                                 |
| $MACRO tutti                                    |
...
**Note**: Some interrupts cannot be disabled, e.g. Power fail.
...
| OPCOM only         |            | with DOMINOS       |            |
|--------------------|------------|--------------------|------------|
| address            | supervisor | user     | address  | supervisor | user     |
| 000000H            |            |          |          |            |          |
|                    | read only  | no access|          | →          |          |
| 400H               |            |          |          |            |          |
...
It is quite easy to simulate an INISTACK statement for the stack layout belonging to "OPTION 2", using an array dynamically allocated on the machine stack! This is exactly what the standard INISTACK lacks!
...
The environment for handlers of this kind of traps is quite similar to that of interrupt handlers. The main difference is that, since the trap is programmed, the programmer may define an interface between the handler and the trap-producing activity. The programmer controls both sides of the trap. By hiding the trap-producing activity inside a routine (e.g., a library) this interface need not be known anywhere else. Trap implementation in DOMINOS shows this quite clearly:
...
Which DOMINOS process services may be called from a trap handler depends on which environment the handler has been called from. If it was a process or a different trap handler (UDS and PME), any service may be called. If, however, it was an interrupt handler, which activated a trap handler, only PIRSetEvent and PIRFatal may be called. This applies also to nested trap handlers. The lowest level is important!
...
**Process stack**  
Inside DOMINOS, the PLANC stack of the calling process is used. Each process should therefore keep about 1/2 KByte of extra stack space plus that amount used by the PME (and even more if the UDS requires more than that).

**ERRETURN**  
Use of ERRETURN inside DOMINOS is not allowed. Owing to the memory-protection system usage, the pg; error handler will reside in a memory area which has no fetch permission in supervisor mode. This results in a fatal error when ERRETURN is executed.
...
|    0 means return pointer to process description     |
|    currently running (i.e. the process calling the   |
|    UDSE).                                            |
...
I'm sorry, but the page is blank, so there's no content to convert to Markdown.
...
# Chapter 6 NUCLUS Overview

## NUCLEUS Kernel

NUCLEUS data structures reside in shared memory, operated upon by specific rules. Parts of physical memory are reserved for the data structure used by NUCLEUS.
...
For ND-5000, the time-critical NUCLEUS calls nkMove, nkSend, nkReceive and nkGetInfo are microcoded to achieve required performance. All other NUCLEUS calls are executed in ND-100.

For ND-500, the time-critical NUCLEUS calls are not microcoded. These calls are executed in ND-100 (level 12). The NUCLEUS library in ND-500/5000 presents a standard NUCLEUS interface for applications.
...
The NUCLEUS Monitor is a tool for inspection of tables and queues in the NUCLEUS kernel.
...
| EXTENSION ON (user-area)nk-dom-os-c,       |
| (user-area)pl-mc68020                      |
| ENDEXT                                     |
| PROCESS ON                                 |
| test,(user-area)nk-dom-appl-c,             |
| (user-area)PLANC-MC68020                   |
| ENDPROC                                    |
| INSERT 'LIST-ENT ALL'                      |
| ENDCONFIG                                  |
+--------------------------------------------+
...
A message belongs to the user that created it. If a user creates a message, sends it away, logs out, logs in again, and the message still exists, it will still be on this user's account. Public processes cannot bypass the resource restrictions by logging out and in again.

If someone tries to return a message to a home port that does not exist any more (the user may have logged out), the message will be deleted, and subtracted from the user's account.
...
|--------------------------------------------------------|-----------------------------|
| Message buffer space for system processes in pages     | 250 Kbytes\(^1\)            |
|                                                        | (125 pages)                 |
| Number of descriptors for all system processes         | 500\(^1\)                   |
| Message buffer space for all public processes in pages | 250 Kbytes\(^2\)            |
|                                                        | (125 pages)                 |
| Number of descriptors for all public processes         | 300\(^2\)                   |
| Message buffer space per public process in pages       | 10 Kbytes                   |
|                                                        | (5 pages)                   |
| Number of descriptors per public process               | 10                          |
| Trace buffer space in pages                            | 2 Kbytes                    |
|                                                        | (1 page)                    |
...
**NOTE:**
    ...
    ┌───────────────────────────────────────────────────────┐
    │ In calls with only one function,                      │
...
| PORT           | function = 0 ; nkfOpenHomePort                                         |
|                | function = 1 ; nkfOpenLastPort                                         |
| DELETE NAME    | nkDelName(function, name, port)                                        |
| CREATE MESSAGE | nkCreMessage(function, bytes, homeport, =message)                      |
...
nkClose(function,port or message or sendreference)
    ...
    function = 1 ; nkfReject    │ messages
```

## GET INFO

```
nkGetInfo(function,port or message or
...
¹ Subfunction names are valid for messages only.
...
| nke_PROTOCOLERROR| 101020b   | Inconsistent Nucleus module versions installed                 |
...
| nke_QUOTAAUSED       | 101040b   | No more Nucleus resources available for this user      |
...
|                       | nkCreMessage | nkVersion | nkOpenReturnPort | nkGetInfo | nkOpenPort | nkClose | nkCreName | nkReceive | nkSend | nkCrePort | nkMove | nkDelName |
|-----------------------|--------------|-----------|------------------|-----------|-----------|---------|-----------|-----------|--------|-----------|--------|-----------|
| nke_BUFFULL           |              | X         |                  |           |           |         |           |           |        |           |        |           |
| nke_DELAYTABFULL      | X            |           |                  |           |           |         |           |           |        |           |        |           |
| nke_DESCARRFULL       | X            |           |                  |           |           |         |           |           |        |           |        |           |
| nke_FATAL             | X            | X         | X                | X         |           | X       |           | X         |        | X         |        |           |
| nke_ILLFUNC           | X            | X         | X                | X         | X         |         | X         | X         |        | X         |        | X         |
| nke_ILLHARDWARE       | X            | X         | X                | X         | X         |         | X         | X         |        | X         |        | X         |
| nke_ILLKERNELNO       |              | X         | X                | X         | X         |         | X         | X         |        | X         |        | X         |
| nke_ILLNETADDRESS     |              | X         |                  |           | X         |         |           | X         |        |           |        |           |
| nke_ILLNO             | X            |           | X                |           | X         | X       | X         | X         | X      | X         | X      | X         |
| nke_ILLPAR            |              | X         |                  |           | X         |         |           | X         |        |           |        |           |
| nke_ILLTYPE           | X            | X         | X                | X         | X         |         | X         | X         |        | X         |        | X         |
| nke_ILLUSER           |              | X         |                  |           |           |         |           |           |        |           |        |           |
| nke_ILLVERSION        | X            | X         | X                | X         | X         |         | X         | X         | X      | X         |        | X         |
| nke_INCONSISTENT      |              |           | X                | X         | X         |         |           | X         |        |           |        | X         |
| nke_KICKLOCK          |              |           |                  | X         |           |         |           |           |        |           |        |           |
| nke_LOCK              |              |           | X                | X         | X         |         |           | X         |        |           |        | X         |
| nke_NAMEFULL          |              |           |                  | X         |           |         |           | X         |        |           |        |           |
| nke_NAMEINDEXERROR    |              |           |                  | X         |           |         |           |           |        |           |        |           |
| nke_NAMENOTFOUND      |              |           |                  | X         |           | X       | X         |           | X      | X         |        |           |
| nke_NAMEPORTUSED      |              |           |                  |           |           |         |           |           |        | X         |        |           |
| nke_NAMEUSED          |              |           |                  |           |           | X       |           |           | X      | X         |        |           |
| nke_NETTABFULL        |              |           |                  |           | X         |         |           |           |        |           |        |           |
| nke_NOACCESS          |              | X         | X                |           | X         |         | X         | X         | X      | X         | X      | X         |
| nke_NOMESS            |              |           |                  |           | X         |         | X         | X         |        | X         |        |           |
| nke_NOTAVAILABLE      |              |           |                  |           |           |         | X         |           | X      |           |        |           |
| nke_NOTEVENBYTE       |              |           |                  |           |           |         |           |           |        |           |        | X         |
...
|              | 1      | Delay abort. `nkfDelayAbort` For further information about `nkfDelayAbort`, see next page.          |
| `<events>`   | ≠ 0    | If ND-100 or ND-500: The process will be activated when the first message arrives at the empty port.|
|              |        | **ND-100**: Process is stopped by MON 267 (TimeOut).                                                |
|              |        | **ND-500**: Process is stopped by MON 501 (StopProcess) or MON 514 (ND500TimeOut).                  |
|              |        | If DOMINO: Events will be used together with the event system in DOMINOS.                           |
|              |        | Event bit 30 and 31 are used by NUCLEUS itself. These bits cannot be used by any application!       |
...
**NOTE**  
This subfunction is yet only available for ND-5000 System processes.

### Example

In some cases DOMINO operates directly on fixed segments of ND-5000 processes. It is important that the process is not aborted (and the segments unfixed) while DOMINO carries out data transfers. To avoid this, process abortion should be delayed while data transfer control messages still remain.
...
|------------|-------------|
| `<function>` | = 0 |
| `<access>`   | = 0 Only System processes have access to this port. <br> = 1 System and public access. |
...
**Syntax**  
```
nkMove(<function>,<message>,<displacement>,
       <(=)data>,<~bytes>)
```
...
| Parameter        | Description                                                                 |
|------------------|-----------------------------------------------------------------------------|
| `<function>`     | = 0 => Read message. `nkfRead`.                                             |
|                  | = 1 => Write message. `nkfWrite`.                                           |
|                  | = 2 => Insert. Same function as Write, but the byte pointer is not set if the message is smaller than the old message. `nkfInsert`. |
...
| 2            | `nkfHomeid`. <br>If message: Home port identifier. <br>If port: Port identifier. <br>If send reference: Destination port identifier. |
...
- `<function> = 2 and 3` returns 64 bits (8 bytes).  
  For future NUCLEUS extension, all applications must be prepared for returning 128 bits (16 bytes).
...
| NOTE !                                                        |
| If <function> = 2 or 3, the identifiers returned can only be  |
| used to compare other identifiers returned from nkGetInfo.    |
...
|----------------|--------------------------------------|
| \<message      | = Message number.                    |
| or port or     | = Port number.                       |
| sendreference\>| = Sendreference number.              |
| \<=value\>     | = Message, port or sendreference information. |
...
| nke_ILLTYPE         | % Invalid descriptor type                                      |
| nke_ILLVERSION      | % Invalid version of NUCLEUS library.                          |
| nke_NOACCESS        | % No access to port                                            |
| nke_NOTINITIALISED  | % NUCLEUS not started                                          |
...
IF lastport in message is set and not closed THEN
    ...
    ENDIF
ELSE
    IF homeport closed or owned by invoking process THEN
        deallocate message
    ELSE
...
Closing a port results in deletion of the port number and all of the ports symbolic names.  
If there exist messages (in queue to the port) that the port has not yet received, the messages will be closed according to the algorithm above (function = 1 [nkfReject]).
...
| NOTE!                         |
|                               |
| When a process is aborted or  |
| a CPU in the system is        |
| rebooted, messages are        |
| deallocated/closed as in      |
| function = 1 (see next page). |
...
- `<function> = 1`
  - `[nkfReject]`: The message is closed according to the algorithm for closing a message.
...
| Parameter      | Description                                                                                   |
|----------------|-----------------------------------------------------------------------------------------------|
| `<function>`   | `= 0 =>` Version of NUCLEUS library (NUCLEUS library application is linked to). [nkfLibrary]  |
...
| `<version>`    | Version consisting of three alphanumeric characters.                                          |
...
| `nke_ILLVERSION`   | % Invalid version of NUCLEUS library.         |
| `nke_NOTAVAILABLE` | % Domino controller not yet started.          |
...
```mermaid
graph TD
  A[MASTER BLOCK] --> B[POINTER TO DESCRIPTOR TABLE]
  A --> C[POINTER TO HASH ARRAY]
  A --> D[HASH MASK]
  A --> E[POINTER TO KICK TABLE]
  A --> F[POINTER TO NET ADR TABLE]
  A --> G[POINTER TO START OF BUFFER AREA]
  A --> H[POINTER TO END OF BUFFER AREA]
  B[MASTER BLOCK] --> I[DESCRIPTOR TABLE]
  C --> J[HASH ARRAY]
  E --> K[KICK TABLE]
  F --> L[NET TABLE]
  G --> M[BUFFER AREA]
...
_Figure 23. Tables in NUCLEUS kernel_

```plaintext
+--------------+-----------------------------+
| LOCK         | --> Used for TSET           |
| 2            | --> TYPE= 2 => Message      |
| HEAD OWNER   | --> Owner of message. Used  |
|              |     for access check        |
...
_Figure 24. Record layout for a message in descriptor table_
...
| 3        | TYPE= 3 => Port               |
| HEAD     |                               |
| OWNER    | Owner of port. Used for       |
|          | access check                  |
| FREELINK | Link in freelist              |
| USER     | Used for quota control        |
| MESS     | Start of message queue        |
| HEAD     |                               |
| MESS     | End of message queue          |
| TAIL     |                               |
| KICKLINK | Link kicked ports together    |
| KICK     | Points to queue head in       |
| HEAD     | kicktable                     |
| KICK     | Index in kicktable            |
| DEST.    | (=OCTOBUS station no)         |
| INQUEUE  | = 0 => NOT in kickqueue       |
| KICK     | Process to be kicked          |
| PROC.    |                               |
| EVENTS 1 | Events to be set              |
| EVENTS 2 | More event info               |
| OWNINDEX | Descriptor number             |
| PRANDOM  | Magic number                  |
| NETTADDR | To complete identifier        |
| OPENCOUNT| Number of times opened for    |
|          | receive                       |
...
_Figure 25. Record layout for a homeport in descriptor table_
...
## Record Layout for a Sendreference in Descriptor Table
...
```
 ...
 |_______________________|
 |        HEAD           |
 |       OWNER           |  --> Owner of sendref. Used for access check
 |_______________________|
 |     FREELINK          |  --> Link in freelist
 |_______________________|
 |        USER           |  --> Used for quota control
 ...
 |_______________________|
 |  DESTINATION PORT     |  --> Destination pointer
 |_______________________|
```

**Figure 26.** Record layout for a sendref in descriptor table

## Message Buffer Layout in Buffer Area
...
**Figure 27.** Message buffer layout in buffer area
...
```mermaid
flowchart LR
    A[PROCESS A SERVER] -->|nkCrePort| B[NK-LIB]
    A -->|nkCreName| F[NUCLEUS NAME SERVER]
    B -->|1| C[NUCLEUS kernel]
    F -->|2| C
    C -->|3| D[NK-LIB]
    D -->|nkCrePort| E[PROCESS B CLIENT]
```

_Figure 28. Creating ports and names in NUCLEUS_
...
## Create Message and Open Port

```mermaid
flowchart LR
    subgraph PROCESS_A_NK
    direction TB
        SERVER_LIB
    end
    ...
    NUCLEUS ---> |NUCLEUS NAME SERVER| PROCESS_B
    PROCESS_B -->|nkCreMessage (4)| 
    PROCESS_B -->|nkOpenPort (5)|
```

_Figure 29. Create message and open port_

1. Process B (client) creates a message (`nkCreMessage`)
2. Process B (client) opens the port created by Process A to get a sendreference to the port.
...
## Write a Message into the Message Buffer
...
```mermaid
flowchart LR
    PROCESSA[PROCESS A<br>SERVER<br>NK-<br>LIB]
    NUCLEUS[NUCLEUS<br>kernel]
    PROCESSB[PROCESS B<br>NK-<br>LIB<br>CLIENT]

    PROCESSA -- 8) nkReceive --> NUCLEUS
    NUCLEUS --> |1| MessageQueue1[ ]
    NUCLEUS -.-> |3| MessageQueue3[ ]
    NUCLEUS --> MessageBuffer["Message buffer"]
    MessageBuffer --> PROCESSA

    PROCESSA -- 9) nkMove --> NUCLEUS
    NUCLEUS --> |3| MessageQueue3_2[ ]
    NUCLEUS --> MessageBuffer2["Message buffer"]
    MessageBuffer2 --> PROCESSA
```
...
```mermaid
flowchart TB
    subgraph Descriptor_3
        direction TB
        D3_LOCK(LOCK)
        D3_HEAD(2)
        D3_ProcessB(Process B)
        D3_Freelink(Freelink)
        D3_User(User)
        D3_LINK(LINK)
        D3_BUFFERPOINTER(BUFFERPOINTER)
        D3_HOMEPORT(HOMEPORT)
        D3_HASHLINK(HASHLINK)
        D3_COMSTAT(COMSTAT)
        D3_DescrNo(3 (=Descr. #))
        D3_TRACECOND(TRACECOND)
    end
    
    subgraph Descriptor_2
        direction TB
        D2_LOCK(LOCK)
        D2_HEAD(HEAD)
        D2_3(3)
        D2_ProcessB(Process B)
        D2_Freelink(Freelink)
        D2_User(User)
        D2_MESSHEAD(MESS HEAD)
        D2_MESSTAIL(MESS TAIL)
        D2_KICKLINK(KICKLINK)
        D2_KICKHEAD(KICK HEAD)
        D2_KICKDEST(KICK DEST.)
        D2_INQUEUE(INQUEUE)
        D2_KICKPROC(KICK PROC.)
        D2_EVENTS1(EVENTS 1)
        D2_EVENTS2(EVENTS 2)
        D2_DescrNo(2 (descr. #))
        D2_PRANDOM(PRANDOM)
        D2_NETADDRESS(NETADDRESS)
        D2_OPENCOUNT(OPENCOUNT)
        D2_NAMED(NAMED)
    end
    
    subgraph Descriptor_1
        direction TB
        D1_LOCK(LOCK)
        D1_HEAD(HEAD)
        D1_3(3)
        D1_ProcessA(Process A)
        D1_Freelink(Freelink)
        D1_User(User)
        D1_MESSHEAD(MESS HEAD)
        D1_MESSTAIL(MESS TAIL)
        D1_KICKLINK(KICKLINK)
        D1_KICKHEAD(KICK HEAD)
        D1_KICKDEST(KICK DEST.)
        D1_INQUEUE(INQUEUE)
        D1_KICKPROC(KICK PROC.)
        D1_EVENTS1(EVENTS 1)
        D1_EVENTS2(EVENTS 2)
        D1_OWNID(OWN ID)
        D1_DescrNo(1 (descr. #))
        D1_PRANDOM(PRANDOM)
        D1_NETADDRESS(NETADDRESS)
        D1_OPENCOUNT(OPENCOUNT)
        D1_NAMED(NAMED)
    end
    
    subgraph Descriptor_4
        direction TB
        D4_LOCK(LOCK)
        D4_HEAD(HEAD)
        D4_4(4)
        D4_ProcessB(Process B)
        D4_Freelink(Freelink)
        D4_User(User)
        D4_DESTPORT(DESTINATION PORT)
    end
    
    D3_ProcessB -->|message owner| D1_ProcessA
    D2_DescrNo -->|Port Owner| D1_ProcessA
    
    D2_OWNID -- OWNID --> D1_OPENCOUNT
    
    D4_4 -->|DESTINATION PORT| D1_OPENCOUNT
    D1_OPENCOUNT -->|MESSAGE AREA| D1_nor
```

_Figure 34: Pointers in descriptor table_
...
$INCLUDE (user-area)nk-library:tmpt

INTEGER ARRAY : stack (0:9999)

% server data:
INITIAL: S1RecPort % port number to receive message
INITIAL: S2RecPort % port number to receive message
INITIAL: S1Sendref % sendreference for serverA
INITIAL: S2Sendref % sendreference for serverB
INTEGER: smess % message number
INTEGER: smoa2d % number of bytes
INITIAL: smesslength % message length
BYTES: quest(0:29) % received data

% user data:
INITIAL: URecPort % port number to receive message
INTEGER: umess % message number
INITIAL: USendref % sendreference
INTEGER: lmoa2d % number of bytes
INITIAL: umesslength % message length
...
nkCreatePort(0,2,SIRecPort) chstat
nkCreateName(0,1,Adt# 'serverA', SIRecPort) chstat

% USER ESTABLISH CONNECTION
nkCreatePort(0,2,URecPort) chstat
nkCreateMessage(0,50,URecPort,umess) chstat
nkOpenPort(0,Adt# 'serverA', USendref) chstat

% USER REQUEST
nkMove(1,umess,0,Adt# 'ask serverA', bmoved) chstat
IF bmoved >< 11 THEN 1 ERRRETURN ENDIF
nkSend(0,0,USendref,umess) chstat

% SERVER READS REQUEST
nkReceive(0,SIRecPort,smess,smesslength) chstat
IF smesslength >< 11 THEN 1 ERRRETURN ENDIF
nkMove(0,smess,0,Adt# quest,bmoved) chstat
IF bmoved >< 11 OR quest >< 'ask serverA' THEN 1 ERRRETURN ENDIF

% SERVER RESPONSE
% SEND MESSAGE TO HOMEPORT OF MESSAGE
nkMove(1,smess,0,Adt# 'answer from serverB', bmoved) chstat
IF bmoved >< 19 THEN 1 ERRRETURN ENDIF
nkSend(0,0,0,smess) chstat

% USER GETS RESPONSE
nkReceive(0,urecport,umess,umesslength) chstat
IF umesslength >< 19 THEN 1 ERRRETURN ENDIF
nkMove(0,umess,0,Adt# response, bmoved) chstat
IF (bmoved >< 19) OR (response >< 'answer from serverB') THEN 1 ERRRETURN
ENDIF
...
% SERVER DISCONNECT
nkDeleteName(0,Adt# 'testserv', SIRecport) chstat
nkClose(0,SIRecport) chstat
...
$INCLUDE (user-area)rk-library:fmpt

INTEGER ARRAY : stack (0:9999)

% server data:
INTEGER: S1RecPort % port number to receive message
INTEGER: S2RecPort % port number to receive message
INTEGER: S1UsrSendRef % sendreference for ServerA to User
INTEGER: S1Sendref % sendreference for ServerA
INTEGER: S2Sendref % sendreference for ServerB
INTEGER: snmess % message number
INTEGER: snmoved % number of bytes
INTEGER: smesslength % message length
BYTES: quest(0:29) % received data

% user data:
INTEGER: URecPort % port number to receive message
INTEGER: unmess % message number
INTEGER: Usendref % sendreference
INTEGER: unmoved % number of bytes
INTEGER: umesslength % message length
BYTES: response(0:29) % received data
```
...
- **ServerA establish connection**
  ```
  nl¢CreaPort(0,2,SlRecPort) chstat
  nl¢CreaName(0,1,Add# 'ServerA',SlRecPort) chstat
  ```

- **ServerB ESTABLISH CONNECTION**
  ```
  nl¢CreaPort(0,2,S2RecPort) chstat
  nl¢CreaName(0,1,Add# 'ServerB',S2RecPort) chstat
  ```

- **USER ESTABLISH CONNECTION TO ServerA**
  ```
  nl¢OpePort(0,2,URecPort) chstat
  nl¢CreaMessage(0,50,URecPort,umess) chstat
  nl¢OpePort(0,Add# 'ServerA',USendref) chstat
  ```

- **USER REQUEST TO ServerA**
  ```
  nl¢Move(l,Umess,0,Add# 'ask ServerA',bmove) chstat
  IF bmove.x 11 THEN 1 ERRORRUN ENDIF
  nl¢Send(0,URecport,USendRef,Umess) chstat
  ```

- **ServerA READ REQUEST**
  ```
  nl¢Receive(0,SlRecPort,umess,$MessLength) chstat
  nl¢OpeReturnPort(1,Umess,S1UserSendRef) chstat
  IF $MessLength.x 11 THEN 1 ERRORRUN ENDIF
  nl¢Move(0,umess,0,Add# quest,shmove) chstat
  IF shmove.x 11 OR quest >< 'ask ServerA' THEN 1 ERRORRUN ENDIF
  ```

- **ServerA ESTABLISH CONNECTION TO ServerB**
  ```
  nl¢OpePort(0,Add# 'ServerB',S1Sendref) chstat
  ```

- **ServerA REQUEST TO ServerB. ServerA SENDS ITS OWN RECEIVE PORT AS LAST SENDER.**
  ```
  nl¢Move(1,umess,0,Add# 'ask ServerB',bmove) chstat
  IF $MessLength.x 11 THEN 1 ERRORRUN ENDIF
  nl¢Send(0,SlRecport,S1SendRef,umess) chstat
  ```

- **ServerB READ REQUEST FROM ServerA**
  ```
  nl¢Receive(0,S2RecPort,umess,$MessLength) chstat
  IF $MessLength.x 11 THEN 1 ERRORRUN ENDIF
  nl¢Move(0,umess,0,Add# quest,shmove) chstat
  IF shmove.x 11 OR quest >< 'ask ServerB' THEN 1 ERRORRUN ENDIF
  ```

- **ServerB ESTABLISH CONNECTION TO ServerA**
  - **OPEN A SEND REFNAME TO PORT SET A LAST PORT IN MESSAGE.**
    ```
    nl¢OpeReturnPort(1,Umess,S2SendRef) chstat
    ```
...
```
nkMove(1, umess, 0, Adit+ 'answer from ServerB', stmovea)   chstat
IF stmovea >< 19 THEN 1 ERRETURN ENDIF
nkSend(0, 0, S2SendRef, umess) chstat
```

## % ServerA READ REQUEST FROM ServerB
```
nkReceive(0, S1RecPort, umess, S1messLength) chstat
IF S1messLength >< 19 THEN 1 ERRETURN ENDIF
nkMove(0, umess, 0, Adit+ quest, stmovea) chstat
IF (stmovea >< 19) OR (quest >< 'answer from ServerB') THEN 1 ERRETURN ENDIF
```

## % ServerA RESPONSE USER
```
nkMove(1, umess, 0, Adit+ 'answer from ServerA', stmovea) chstat
IF stmovea >< 19 THEN 1 ERRETURN ENDIF
nkSend(0, 0, S1UserSendRef, umess) chstat
```

## % USER GEMS RESPONSE FROM ServerA
```
nkReceive(0, URecPort, umess, SmessLength) chstat
IF SmessLength >< 19 THEN 1 ERRETURN ENDIF
nkMove(0, umess, 0, Adit+ quest, stmovea) chstat
IF (stmovea >< 19) OR (quest >< 'answer from ServerA') THEN 1 ERRETURN ENDIF
```
...
nkClose(0, S1SendRef)
nkDeName(0, Adit+ 'ServerA', S1RecPort)
nkClose(0, S1RecPort)
```
...
```
nkClose(0, S2SendRef)
nkDeName(0, Adit+ 'ServerB', S2RecPort)
nkClose(0, S2RecPort)
```
...
INTEGER: S1RecPort % port number to receive message
INTEGER: S2RecPort % port number to receive message
INTEGER: S1Sendref % sendreferance for ServerA
INTEGER: S2Sendref % sendreferance for ServerB
INTEGER: smess % message number
INTEGER: bmove1 % number of bytes
INTEGER: smesslength % message length
BYTES: quest(0:29) % received data
...
INTEGER: URecPort % port number to receive message
INTEGER: umess % message number
INTEGER: USendref % sendreferance
INTEGER: bmove2 % number of bytes
INTEGER: umesslength % message length
BYTES: response(0:29) % received data
```
...
% ServerA ESTABLISH CONNECTION  
nkCrePort(0,2,SlRecPort) chstat  
nkCreName(0,1,Addr 'ServerA',SlRecPort) chstat  
...
% USER REQUEST TO ServerA  
nkMove(1,umess,0,Addr 'ask ServerA',bmove8) chstat  
IF bmove8 .X. 11 THEN 1 ERRRETURN ENDIF  
nkSend(0,0,USendRef,umess) chstat  

% ServerA READ REQUEST  
nkReceive(0,SlRecPort,umess,$MessLength) chstat  
IF $MessLength .X. 11 THEN 1 ERRRETURN ENDIF  
nkMove(0,umess,0,Addr quest,smove8) chstat  
...
nkClose(0,Umess)  
nkDeName(0,Addr 'ServerA',SlRecPort)  
nkClose(0,SlRecPort)  
...
The page is blank except for the page number at the top and a footer.
...
```mermaid
flowchart TD
    A[ERROR device] --> C[S3WatchDog]
    B[Inspect-log] --> D[Ring file]
    
    subgraph ND-100
        C
        D
        F[PROMAN<br/>(PROcessor MANager)]
    end
    ...
    subgraph MultiPort Memory
        J[NUCLEUS<br/>KERNEL<br/>DATA]
    end
        
    subgraph OCTOBUS
        J
        P[MultiPort Memory]
        I[DOMINO<br/>controller<br/>N]
        Q[OPCOM] --> R[DOMINOS]
        R --> S[NUCLEUS]
    end
    ...
    subgraph NUCLEUS
        T[ERS<br/>LIB]
        U[DOMINO<br/>application<br/>software]
    end
...
NUCLEUS checks if the NUCLEUS version number in each CPU is correct. In case of a version mismatch, an error message is sent to the error device.
...
```mermaid
flowchart LR
    A[Background program in ND-100] --> B[NKS-VERIFY]
    B --> C[NKS-LOGFILE:LOGS]
    B --> D((NUCLEUS ND-100))
    B --> E((NUCLEUS ND-500))
    B --> F((NUCLEUS DOMINO))

    D --> |NKS100:PROG| G[Verify func.\nVerify error\nVerify comm.\nStress fast\nStress slow]
    E --> |NKS500:DOM| H[Verify func.\nVerify error\nVerify comm.\nStress fast\nStress slow]
    F --> |NKSDOMINO:IMAG| I[Verify func.\nVerify error\nVerify comm.\nStress fast\nStress slow]
``` 
...
|   NOTE !                       |
| Stress modules are not         |
| implemented in C version       |
|________________________________|
```

## Function Keys

| Function      | NOTIS Terminal | non-NOTIS Terminal |
|---------------|----------------|--------------------|
| F1            | CTRL+D twice   |
| F2            | CTRL+L         |
| F3            | FUNC CTRL+U    |
...
Simultaneous verification in ND-100, ND-500 and DOMINO is allowed. Verification can only be performed in one DOMINO at the time.
...
I'm unable to convert the scanned page as it appears to be blank or too faint to read. If you have another page or image, please provide it.
...
**Promt:** The NUCLEUS monitor prompts with `nkm:` whenever it is ready to accept a command. You may now use the commands on the high-level. If the commands you need are on the low-level (advanced mode), give the command:

    nkm:ADVANCED-MODE.␍

The monitor prompt changes to `nkm(adv):`

**Notation:** When describing the commands available in the NUCLEUS monitor, the following rules apply:
...
**Command entering:** As in SINTRAN.

**Radix:** Numeric arguments may be given in octal, decimal, or hexadecimal. The default radix is octal, but may be changed by the Main-format command. A trailing `B` (octal) or `D` (decimal) overrides the format.
...
It may look as if a port has more than one name, especially if a process fails to terminate properly. In most cases, this is not true, as the random number part of the port number is different.
...
| values for port number,   |
| the current port number   |
| is always the default     |
| port number. If the de-   |
| fault is not used, the    |
| given port number becomes |
| the current port number.  |
| This also applies for     |
| message and sendreference |
|___________________________|
```
...
A sendreference number is returned, and can be used to send to \<portname\>.  
The sendreference number returned becomes current sendreference.
...
Dump the NUCLEUS kernel to `<file name>`.  
**Parameter:** File name. See also the related command CONNECT-FILE on page 244.
...
Parameter: extra format string. HDOA or any combination of them may be used.
...
_Figure 38. Image file header versus image area_
...
All selftests (except preboot) follow the defined layout using TDF's. A short description of all the elements in the TDF record is given below.
...
```
CONSTANT OkKey = 12421043O40B   % Ascii 'TDF '
```
...
| Test Name | Test No/ | Description               |
|-----------|---------|---------------------------|
| WarmCold  | 400H    | Start checking for warm or cold start. |
| AddrInAddr| 500H    | Start of memory test.     |
|           | 501     | Error found in RAM        |
|           |         | - Problems with timing against DRAM? Missing RAS, CAS RW etc. |
|           |         | - Problems with data or address bus? |
|           |         | - Problems with one of the memory packages? |

### Sizetest

| Test No/ | Description               |
|----------|---------------------------|
| 600H     | Start of byte selection test. |
|          | All combinations of byte, word, long, read, and write is tested. |
|          | - Problems with the RW pal that generates write strobes to the DRAM? |
|          | - Problems with timing against DRAM? Missing RAS, CAS RW etc. |
...
| Test No/ | Description               |
|----------|---------------------------|
| 700H     | Start of parity test.     |
| 701      | Unexpected Parity Error   |
|          | A parity error occurred immediately after enabling the parity system. |
|          | - Problems somewhere in the parity network. |

### ParityNet

| Test No/ | Description               |
|----------|---------------------------|
| 800H     | Start of parity check for each byte. |
| 804      | No interrupt from byte 0  |
|          | - Problems in parity network for byte 0? |
| 805      | No interrupt from byte 1  |
|          | - Problems in parity network for byte 1? |
| 806      | No interrupt from byte 2  |
|          | - Problems in parity network for byte 2? |
| 807      | No interrupt from byte 3  |
|          | - Problems in parity network for byte 3? |
...
| 3201 | Not valid EEPROM test pattern.                         |
|      | - No EEPROM?                                           |
...
The page is blank and contains no identifiable text or diagrams except for the page number and footer.
...
| 105004B   | Image file is empty, booting aborted                         | ERROR |
|           | Controller at station..: I2O                                 |       |
| 105005B   | Unrecognised event in Domino boot-session ignored            | WARN. |
|           | Controller at station..: I2O                                 |       |
...
| 105013B   | Unrecognised Domino-session event ignored                    | WARN. |
|           | Controller at station..: I2O                                 |       |
...
| 105047B   | Domino controllers restarted after powerfail <br> Number restarted.......: I2D <br> Number rebooted.........: I2D | INFO  |
| 105050B   | Too small ERS-buffer fill, must at least be two pages long                                     | WARN. |
| 105051B   | Domino controller has been terminated on request <br> Controller at station..: I2O             | INFO  |
...
| Octal | Meaning                                                                 | TYPE  |
|-------|-------------------------------------------------------------------------|-------|
| 6006B | Unable to start Dominos                                                  | FATAL |
|       | Octobus station.............. I2O                                       |       |
|       | Crate Id (MFB contr. station) I1D                                       |       |
|       | MFB slot.......................... I1D                                  |       |
|       | Exception number........... I2unused                                    |       |
|       | Address where occurred.... I4H                                          |       |
|       | Dominos error.................. SEC                                     |       |
| 6007B | Error reported by PIRFatal                                              | FATAL |
|       | Octobus station.............. I2O                                       |       |
|       | Crate Id (MFB contr. station) I1D                                       |       |
|       | MFB slot.......................... I1D                                  |       |
|       | Exception number........... I2unused                                    |       |
|       | Address where occurred.... I4H                                          |       |
|       | Application error............. SEC                                      |       |
...
|-------|-------------------------------------------------------------------------|-------|
|       | BOPCOM                                                                  |       |
| 6260B | BOPCOM: Server started                                                  | INFO  |
|       | ?? I2Unused                                                             |       |
|       | ?? I2Unused                                                             |       |
|       | Version................... 34A                                          |       |
| 6261B | BOPCOM: Path opened to controller                                       | INFO  |
|       | Octobus station........... I20                                          |       |
|       | Message device........... I20                                           |       |
| 6262B | BOPCOM: Path released                                                   | INFO  |
|       | Octobus station........... I20                                          |       |
|       | Message device........... I20                                           |       |
| 6271B | BOPCOM: Too many Octobus errors                                         | FATAL |
| 6272B | BOPCOM: Too many Superkernel errors                                     | FATAL |
| 6273B | BOPCOM: Unable to open own superports **(XMSG not started?)**           | FATAL |
|       | ?? I2Unused                                                             |       |
|       | ?? I2Unused                                                             |       |
|       | ?? I2Ounused                                                            |       |
|       | ?? I12unused                                                            |       |
...
|                        | nkCrePort | nkCreName | nkOpenPort | nkOpenReturnPort | nkCreMessage | nkDelName | nkReceive | nkSend | nkMove | nkClose | nkGetInfo | nkVersion |
|------------------------|-----------|-----------|------------|------------------|--------------|-----------|-----------|--------|--------|---------|-----------|-----------|
| **nke_BUFFULL**        |           |           |            |                  |              |           |           |        |        |         |           | x         |
| **nke_DELAYTABFULL**   | x         |           |            |                  |              |           |           |        |        |         |           |           |
| **nke_DESCARRFULL**    | x         |           |            |                  |              |           |           |        |        |         |           |           |
...
| **nke_ILLHARDWARE**    |           |           |            |                  |              |           |           | x      |        |         |           |           |
| **nke_ILLKERNELNO**    |           |           |            |                  |              |           |           | x      |        |         |           |           |
| **nke_ILLNETADDRESS**  |           | x         |            |                  | x            |           | x         | x      | x      | x       |           | x         |
| **nke_ILLNO**          | x         | x         | x          | x                | x            | x         | x         | x      | x      | x       | x         | x         |
| **nke_ILLPAR**         | x         |           |            |                  |              |           |           |        |        |         |           | x         |
| **nke_ILLTYPE**        | x         | x         | x          | x                | x            | x         | x         | x      | x      | x       | x         | x         |
| **nke_ILLUSER**        | x         |           |            |                  |              |           |           |        |        |         | x         |           |
| **nke_ILLVERSION**     | x         | x         | x          | x                | x            | x         | x         | x      | x      | x       | x         | x         |
| **nke_INCONSISTENT**   | x         | x         |            |                  |              |           |           |        |        |         |           |           |
| **nke_KICKLOCK**       |           |           |            |                  |              |           |           |        |        |         |           |           |
| **nke_LOCK**           |           |           |            | x                |              |           |           |        |        |         |           |           |
| **nke_NAMEFULL**       |           |           |            |                  |              |           | x         |        |        |         |           |           |
...
| 101100b | Nucleus Name server started<br>Version....: 34A<br>Size of name table..: 140 | INFO |
| 101101b | Nucleus server started<br>Version....: 34A<br>Cluster Id (ND-100 Octobus station no. 140)<br>Zeropage for multiport memory.......... 140<br>Page start address of kernel within<br>multiport memory.............................. 140<br>Nucleus kernel size in pages........... 140<br>Message buffer space for all<br>system processes in pages............ 140<br>Number of descriptors for all<br>system processes in pages............ 140<br>Message buffer space for all<br>public processes in pages............. 140<br>Number of descriptors for all<br>public processes............................. 140<br>Message buffer per public process<br>in pages............................................ 140<br>Number of descriptors per public<br>process.............................................. 140<br>Trace buffer space in pages............ 140 | INFO |
...
| 101115b| Inconsistent data structure<br>Detected at address....................... I40<br>Called from............................... I40 | FATAL |
| 101116b| Timeout when waiting for lock<br>Called from............................... I40<br>Address of lock.......................... I40 | FATAL |
...
- **Delayed abort, NUCLEUS** . . . . . . . . . . . . . 191
- **delete macro, DOMINO Monitor** . . . . . . . . . . 67
- **delete port name, NUCLEUS** . . . . . . . . . . . 197
- **descriptor** . . . . . . . . . . . . . . . . . . . 179, 181
...
- **DUMP-MACRO command, DOMINO Monitor**  . . . . . . . . 68
...
## MAIN-FORMAT command, DOMINO Monitor
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 80

## Main-Format, NUCLEUS Monitor
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 237

## Master block, NUCLEUS
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 210
...
## Memory Protection, DOMINOS
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 155

## Message Record Layout, NUCLEUS
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 210
...
## MICE-II, DOMINO Monitor
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 57

## Modify Service, DOMINOS
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 124

## Module Number
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 11, 13, 14

## Multiple Parameter, DOMINO Monitor
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 40

## Naming Convention, DOMINOS
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 117

## NEW-USER-CONTEXT command, DOMINO Monitor
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 44

## NEXT command, DOMINO Monitor
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 82

## nkClose, NUCLEUS
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 207

## nkCreMessage, NUCLEUS
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 198

## nkCrePort, NUCLEUS
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 189

## nkCrePortName, NUCLEUS
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 192
...
## Open Return Port, NUCLEUS
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 195

## OPEN-PATH command, DOMINO Monitor
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 46

## Open-port, NUCLEUS Monitor
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 242

## Optional Parameter, DOMINO Monitor
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 40

## OUTPUT-FILE command, DOMINO Monitor
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 44
...
Manual Name: ___________________      Manual Number: ___________________
...
Company: ___________________      Position: ___________________
...
I'm sorry, but the scanned page doesn't contain any visible text or diagrams to convert into Markdown. If you have another document or page with visible content, feel free to share it!

---

## Page 303

I'm sorry, but the document appears to be blank, so there is nothing available to transcribe.

---

## Page 304

[Photo: Color gradation blocks across a background]
