...
## 1.4 ADDRESS PART
1.4.1 Address Windows 5  
1.4.2 Address Windows Decoding on the 5152 6  
1.4.3 Address Windows Decoding on the 5155 8  
1.4.4 Address Conversion 9  
1.4.5 Interleave 11  
...
| 3 START-UP AND INITIALIZATION OF THE MPM-5 SYSTEMS | 53 | 
...
| INTERLEAVE   |           |
| ADDRESS OK   |           |
| ADDRESS WINDOW RAM | 32 BITS |
| DATA DRIVER  | DATA DRIVER  |
| DATA RECEIVERS | DATA RECEIVER |
| ADDRESS RECEIVER|            |
...
_Fig. 1. Block Diagram Twin 16-Bit Port_
...
+--------------------------------+
            |
+--------------------------------+
| Test to see if channel         |
| address is within address      |
| windows of port                |
+--------------------------------+
            |
+--------------------------------+
| Address conversion             |
| to physical bank address       |
+--------------------------------+
            |
+--------------------------------+
| Interleave                     |
...
On the 5155, two 16 K x 1 bit RAMs are used to decode the address limits. One RAM is used for decoding of LOCAL access, i.e., within the MPM-5 bank, and the other RAM is used for decoding of GLOBAL access, i.e., through a line driver module to other banks.
...
|                  |                               |
|------------------|-------------------------------|
| Data passes      | if address bit 2 = '0'        |
| Data passes      | if address bit 1 = '0'        |
| Data passes      | if address bit 1 = '1'        |
...
|                  |                               |
|------------------|-------------------------------|
| Data passes      | if address bit 2 = '1'        |
| Data passes      | if address bit 1 = '0'        |
| Data passes      | if address bit 1 = '1'        |
...
The interleave configurations possible are shown in Fig. 8. The three bits in the two leftmost columns refer to the port control register described in page 17. The *interleave type* refers to the shifting of address bits. The *bank interleave* refers to the physical placement of each location.

| Port Control Reg. | Bit 1 | Bit 6 | Interleave Type | Configuration with Standard PROM (34200) | Configuration with Special PROM |
|-------------------|-------|-------|-----------------|----------------------------------------|---------------------------------|
...
|                   | 1     | 1     | 8 way           | 16-bit channel, 4-bank interleave.     | 16-bit channel, 4-bank interleave. |
|                   | 0     | 0     | 0 way           | 32-bit channel, 1-bank, no bank interleave. |                                 |
...
| 0     | 0     | 0 way           | 32-bit channel, 1-bank, no interleave.    |                                          |
...
- **Bit 3 = 1, Bit 2 = 1**: Bank 3
- **Bit 3 = 1, Bit 2 = 0**: Bank 2
- **Bit 3 = 0, Bit 2 = 1**: Bank 1
- **Bit 3 = 0, Bit 2 = 0**: Bank 0

**Bits 4 and 5** are called 'speed-up'. They may be used if the addresses from the channel sources are stable before request is sent. The table below shows the effect of the different speed-up bit settings.
...
**For 5152i:** Four 4 Kbit RAMs are used in decoding the address windows. 16-bit data is written into the RAMs, and decoded as a 12-bit address and a 4-bit data word. The four RAMs may be considered as a 16K x 1 bit memory, where each address on the channel corresponds to one bit in the memory.

**For 5155i:** Two 16 K x 1 bit RAMs are used in decoding the limits, one for global access and the other for local access. Each channel address corresponds to one bit in memory.
...
- **Bit 11:**
  - = '1' means write enable. Prepares the contents of the Error Memory Data register to be written into the chip. To do the actual transfer, a programming pulse must be sent separately from the universal timer of the maintenance processor.
...
- memory access to this module without regard to the limit registers is enabled and  
...
*Comment: If the answer is YES, the configuration parameters are saved both in the non-volatile memory in the backplain and in the registers on the specified module. If the answer is NO, they are stored only on the module.*
...
Memory areas are opened for access by giving LOWER and UPPER LIMITS. Several non-overlapping areas are allowed. START ADDRESS is the first physical address in the MPM-memory. LIMITS and START ADDRESS are in modules of 128 KByte (0=0B, 1=400000B, 2=1000000B,..., nn*4000000B).
...
> **Comment:**  
> These values for lower and upper limits mean that the address window starts from address 0 and ends at address 000017B, inclusive.

### ACCESS (LOCAL=1, GLOBAL=2, BOTH=3):
1
...
> **Comment:**  
> If holes inside the memory are wanted, add more limits.

### START ADDRESS:
0

> **Comment:**  
> The start address conforms to the base, described on page 11.

### DATALENGTH (16, 32):
16

> **Comment:**  
> The data length refers to bit 6 in the port control register, described on page 17.

### INTERLEAVE TYPE (0, 2, 4, 8):
2

> **Comment:**  
> Interleave type 1 means that you have a 16-bit channel and 1-bank interleave. The interleave type refers to bit 0 and bit 1 in the port control register, described on page 17.

### INTERLEAVE PORT NUMBER (0-3):
0

> **Comment:**  
> The interleave port number refers to bit 2 and bit 3 in the port control register, described on page 17.
...
This command lists the contents of a specified slot in the bank. When the command is given, the program responds as in the following example. The configuration is the same that was created in the CONFIGURE-SLOT example.
...
| Comment:                                                                                                                                             |
|------------------------------------------------------------------------------------------------------------------------------------------------------|
| This register is described on page 17.                                                                                                               |
| When port control register = 140B, it means that                                                                                                     |
| - there is no interleave,                                                                                                                            |
| - the bank has number 0,                                                                                                                             |
| - there is 40 ns request delay,                                                                                                                      |
| - this is a 16-bit channel and                                                                                                                       |
...
| Comment:                                                                                                                                             |
|------------------------------------------------------------------------------------------------------------------------------------------------------|
| This register is described on page 16.                                                                                                               |
| When the master control register = 25B, it means that                                                                                                |
| - requests are not inhibited,                                                                                                                        |
| - supertest is not performed,                                                                                                                        |
| - address bits not supplied by ND-100 are disabled,                                                                                                  |
...
| MPM-5 Bus Description (ND-10.005)                                                                                                                    |
...
### Example:
    ...
    Comment: The memory test will now be performed on the 
    first 256 Kbyte of the MPM-5 memory. The test 
    is run once, and reporting of errors will be 
    suppressed during the test.

---

## 2.9 LOOK-AT-MPMEMORY
...
|---------|-------------------------------|
| 3       | Specification                 |
| 3.1     | Specification for X009.0-291  |
| 3.2     | Specification for X009.1-291  |
| 3.3     | Specification for X012-01     |
| 3.4     | Specification for X012-02     |
| 3.5     | Specification for X012-03     |

## 3.1 Specification for X009.0-291

### Scope

This section covers the specification details specific to the X009.0-291 module.

### Description

- High-performance memory module
- Compatible with standard multiport interfaces
...
Details on the X009.1-291 module specifications.
...
| Parameter        | Description             |
|------------------|-------------------------|
| Operating Temp   | -10 to 45 degrees Celsius|
| Storage Temp     | -50 to 90 degrees Celsius |
...
> **NOTE!** When boards are moved in and out of the crate, _always_ first disable the error investigator with the command DISABLE-ERROR-INVESTIGATOR.
...
The multiport memory system is designed to allow multiple access requests to the memory array simultaneously without interfering with each other. This is achieved through...

### Key Features

- **High Throughput:** Capable of handling multiple data transactions concurrently.
- **Error Correction:** Integrated error correction to ensure data integrity.
- **Scalability:** Easily expandable to accommodate increased data demands.
...
1. **Read Mode:** Allows simultaneous data retrieval requests.
2. **Write Mode:** Supports concurrent data storage operations.

### Conclusion

The multiport memory 5 offers a reliable and efficient solution for systems requiring simultaneous memory access.
...
These bits give the slot number of the controller module.
...
| 11  | BERROR | Bus errors, inclusive of errors |
| 10  | BFATAL | Fatal error, ie., multiple error |
...
The address code is decoded from bits 0-4 in the error-log and represents the MPM5-bus address bits 18-22. With these bus address bits it is possible to locate the rows of chips on the memory board where the faulty chip is sited. When you have both the SYNDROME code and the address code, it is possible to locate the single chip.
...
| ADDRESS code  | ADDRESS code  | Rows of chips  |
|---------------|---------------|----------------|
| (using 1 Mb)  | (using 4 Mb)  | on module      |
| 0             | 0, 1, 2, 3    | L and K        |
| 1             | 4, 5, 6, 7    | J and H        |
...
|---|----|----|----|----|----|----|----|----|----|----|----|----|----|----|----|----|----|----|----|----|
| L | 31 | 35 | 33 | 31 | 29 | 27 | 25 | 23 | 21 | 19 | 17 | 15 | 13 | 11 |  9 |  7 |  5 |  3 |  1 |  0 |
| K | 38 | 36 | 34 | 32 | 30 | 28 | 26 | 24 | 22 | 20 | 18 | 16 | 14 | 12 | 10 |  8 |  6 |  4 |  2 |  0 |
| J | 37 | 35 | 33 | 31 | 29 | 27 | 25 | 23 | 21 | 19 | 17 | 15 | 13 | 11 |  9 |  7 |  5 |  3 |  1 |  0 |
| H | 38 | 36 | 34 | 32 | 30 | 28 | 26 | 24 | 22 | 20 | 18 | 16 | 14 | 12 | 10 |  8 |  6 |  4 |  2 |  0 |
| G | 37 | 35 | 33 | 31 | 29 | 27 | 25 | 23 | 21 | 19 | 17 | 15 | 13 | 11 |  9 |  7 |  5 |  3 |  1 |  0 |
| F | 38 | 36 | 34 | 32 | 30 | 28 | 26 | 24 | 22 | 20 | 18 | 16 | 14 | 12 | 10 |  8 |  6 |  4 |  2 |  0 |
| E | 37 | 35 | 33 | 31 | 29 | 27 | 25 | 23 | 21 | 19 | 17 | 15 | 13 | 11 |  9 |  7 |  5 |  3 |  1 |  0 |
| D | 38 | 36 | 34 | 32 | 30 | 28 | 26 | 24 | 22 | 20 | 18 | 16 | 14 | 12 | 10 |  8 |  6 |  4 |  2 |  0 |
...
MAINT.STAT: 147122 ERRLOG 1: 000001B ERRLOG 2: 031460B ERRLOG 3: 015222B
MASTER: 000016 SLAVE: 000019 ADDRESS: 00001B SYNDROME: 000122B
...
MAINT.STAT:147122 ERRLOG 1:003037B ERRLOG 2:020056B ERRLOG 3:004001B  
MASTER: 000014 SLAVE: 000000 ADDRESS: 000037B SYNDROME:000000B  
```
...
## 7.4 Memory Request and Data

### 7.4.1 Memory Request Cycles

- **Write Request Cycle:**  
  The CPU issues a write request containing the address and data to be written.

- **Read Request Cycle:**  
  The CPU issues a read request with the address from which data is to be read.

### 7.4.2 Timing Diagrams

| Cycle Type        | Signal         | Description                     |
|-------------------|----------------|---------------------------------|
| Write Request     | write_signal   | Indicates data is being written |
| Read Request      | read_signal    | Indicates data is being read    |
...
digit(s) and letter mean position on the card
...
letters and digit mean that these are printed on the card
...
| card edge | Comments |
|-----------|----------|
| * LD 1 RED | PORT IN TEST or NAVIB (not available). NAVIB can be programmed or hardwired XMINH, i.e., port is not connected to any driver. |
...
|  |  | Comments |
|---|---|---|
| 28L | | LOWER LIMIT, least sign. |
| 27L | | LOWER LIMIT, 256 Kbyte units |
| | | SIZE, digit 4 means 1 Mb <br> digit 6 means 4 Mb |

#### Indicators

|  | LD | Color | Description |
|---|---|---|---|
| * | 4 | RED | HARD ERROR, means that the error investigator has found an error that must be corrected for every cycle |
| * | 5 | RED | BAD MEMORY, non-correctable error has been detected within this module |
| * | 1 | YELLOW | CORRECTED, means that at least one error correction is done. Cleared by hard or soft reset, power down or disable/enable switch 1 |
| * | 2 | GREEN | ENABLED, lits when error correction is enabled |

#### Switch

|  | SW | Description |
|---|---|---|
| 24L | 1 | ERROR CORRECTION enable/disable <br> Normal operation: Enabled (switch up) |

#### Additional Indicator

|  | LD | Color | Description |
|---|---|---|---|
| * | 3 | YELLOW | ACCESS, means module is accessed with memory cycles |
...
| Card Edge | Comments |
|-----------|----------|
| 28J       | **SW 2**<br>**LOCAL**<br>Switch up: Allows local cycle, 5152<br>Switch down: Does not allow local cycle |
| 26J       | **SW 1**<br>**GLOBAL**<br>Switch up: Allows global cycle, 5155<br>Switch down: Does not allow global cycle |
...
| Single/Dual MPM-5 | ID NO. | PRINT NO. | ND NO. |
|-------------------|--------|-----------|--------|
...
Company: _____________________________________ Position: ____________
...
| Send to:                  |                                      |
|---------------------------|--------------------------------------|
| Norsk Data A.S            | Norsk Data’s answer will be found on |
| Documentation Department  | reverse side                        |
| P.O. Box 25, Bogerud      |                                      |
| Oslo 6, Norway            |                                      |
