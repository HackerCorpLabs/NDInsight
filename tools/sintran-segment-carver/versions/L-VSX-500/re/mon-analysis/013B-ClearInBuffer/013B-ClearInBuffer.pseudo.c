/* ============================================================================
 * MON 13B  ClearInBuffer (CIBUF)  -  pseudo-C model for an emulator
 * SINTRAN III VSX/500 L.  Shares one body with MON 14B ClearOutBuffer (COBUF).
 *
 * Derived from the real disassembly (see 013B-ClearInBuffer.ASM). Control flow
 * is BYTE-VERIFIED; the semantic labels (which status bit means what, the reset
 * routine at 044222B) are INFERRED from the code structure - treat as a model,
 * not gospel. Addresses in comments are octal.
 *
 * NOTE: the MON 13B -> CIBUF dispatch link is NOT byte-proven; GOTAB[13]=120454B
 * does not point at 044120B. See README "Honest caveats". This models the worker
 * body only, whose attribution rests on the CIBUF symbol name + behaviour.
 * ============================================================================ */

/* Entry 044120 = CIBUF (input ring, MON 13B).
 * Entry 044125 = COBUF (output ring, MON 14B) - one word into the same body.
        ...
        if (A_of(entry) & (1 << 14)) {     /* 044122 BSKP ONE 14 DA (bit 160>>3)  */
            ...
            if ((unsigned)a < (unsigned)T) {    /* 044127 SKP IF DA MLST ST: body if a <u T */
                ...
                if (carry_set())            /* 044133 BSKP ONE 6 STS (C, from the add) */
                    reset_buffer(0044222);  /* 044135 JPL I 65 -> ptr@044222 (indir) */
            }
            ...
            if (carry_set())                /* 044145 BSKP ONE 6 STS (C, from the add) */
                reset_buffer(0044222);      /* 044147 JPL I 53 -> ptr@044222 (indir) */
        }
 ...
 * The shared per-buffer reset routine at 044222B is an indirect JPL I target,
