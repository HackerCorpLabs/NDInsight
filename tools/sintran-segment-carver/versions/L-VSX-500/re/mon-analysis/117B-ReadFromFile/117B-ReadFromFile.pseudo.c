/* ============================================================================
 * MON 117B  ReadFromFile (RFILE)  -  pseudo-C model for an emulator
 * SINTRAN III VSX/500 L.  Shares one body with MON 120B WriteToFile (WFILE).
 *
 * Derived from the real disassembly (see 117B-ReadFromFile.ASM). Control flow
...
/* Entry 102130 = READ (ssk=0, RFILE); entry 102132 (WFILE) = WRITE (ssk=1).
 * One shared body forks on the SSK skip flag. */
int mon_file_xfer(mon_regs *r, int ssk /* 0=read, 1=write */)
{
    frame_setup();                         /* 102133-102136: SAB 17, build B frame */
    ...
    } else {                               /* 102364: READ half (this is MON 117B)  */
...
/* Callers:
 *   MON 117B ReadFromFile:  mon_file_xfer(r, 0);   (entry 102130 RFILE, READ half)
 *   MON 120B WriteToFile:   mon_file_xfer(r, 1);   (entry 102132 WFILE, WRITE half)
 *
 * Parameter record (order per the manual, consistent with the field copies seen):
 *   FileNo, WaitFlag, Buff, BlockNo, NoOfBytes (INTEGER4).
 * ReadFromFile must start on a block boundary; BlockNo = -1 reads the next block.
 */
