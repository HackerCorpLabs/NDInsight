...
itself. **No handler for MON 175B is installed in this SINTRAN L image**, so there is no code
...
so MON 175B has **no dispatch-table entry at all** (it is past the table's end). No worker is
carved or fabricated. All addresses/values are **octal**.

- **No per-call disassembly:** MON 175B is a user-definable slot with no installed handler in
  this image; there is nothing to disassemble, so no `175B-UserDef5.ASM`/`.pseudo.c` is fabricated.
...
```mermaid
flowchart LR
    A["User program<br/>MON 175B"] --> B["ENT14 level-14<br/>T = MON number"]
    B --> C["MGOTA table ends at MON 161B<br/>MON 175B has NO table entry"]
...
`071233B + 175B` = `71430B` lies **past the end of the dispatch table**, inside the adjacent
`D1700`/`E1600` device- and interrupt-ident data block that begins at `071416B`. So MON 175B has
...
|------|------------------------|--------------|-------------|-------|---------|
| word at `071233B+175B` | [commoncode.asm](../../segments-ref/SINTRAN-DATA_commoncode/SINTRAN-DATA_commoncode.asm) · [.hex](../../segments-ref/SINTRAN-DATA_commoncode/SINTRAN-DATA_commoncode.hex) | `71430B` (1 word) | 58928 | `000000` (device/ident-table zero fill) | **documented-absent (zero)** |
| installed handler (site) | — (not in this image) | — | — | after `7ENDC`, via `DEFINE-USER-MONITOR-CALL` | **UNVERIFIED** (none installed) |

**Verify by hand:** `grep '^71430 ' ../../segments-ref/SINTRAN-DATA_commoncode/SINTRAN-DATA_commoncode.hex`
→ byte offset `58928`; then
`dd if=../../../resident/SINTRAN-DATA_commoncode.bin bs=1 skip=58928 count=2 2>/dev/null | od -An -tx1`
→ `00 00` (= octal `000000`). Compare the last real table entry:
...
There is **no instruction body to walk** — that is the point of this entry. MON 175B is one of the
eight user-definable slots and no handler is installed in this SINTRAN L image. The word the naive
`071233B + 175B` index lands on (`000000`) belongs to the neighbouring device/ident
...
Per the manual (`Developer/MON/calls/175B_UserDef5.yaml`): **both input and output parameters are
...
|-------------|-----|---------|---------|
| `T` | in | MON number (`175B`) used to index the level-14 table | VERIFIED (dispatch mechanism) |
...
175B therefore has **no dispatch-table entry**. The word at `71430B` reads `000000` — a zero fill that is **coincidental table data, not a MON 175B handler**.

**What is NOT proven / not present:** any handler body for MON 175B. These are user-definable slots;
