/* ============================================================================
 * MON 220B  DirectOpen (DOPEN)  -  pseudo-C model for an emulator
 * SINTRAN III VSX/500 L.
 *
 * Derived from the real disassembly (see 220B-DirectOpen.ASM). Control flow, the
 ...
 *
 * DirectOpen is one of FOUR entries into a single shared OPEN body at 103041.
 ...
 * the access mode. DOPEN sets M=1,K=1. For public users DirectOpen behaves as
 * OpenFile; user SYSTEM / RT are given the owner's access rights.
 * ============================================================================ */
...
int mon_directopen(mon_regs *r)
{
    /* --- Region A: DOPEN flag-fork prelude (103026-103030) ------------------ */
    STS.M = 1;                             /* 103026: BSET ONE SSM (A |= 1<<M)     */
    STS.K = 1;                             /* 103027: BSET ONE SSK                 */
    goto open_body;                        /* 103030: JMP -> 103041                */
...
open_body:
     ...
     * do_add(0,imm,0) via SAA sets the mode literal (RADD CLD 0 DA = A=0).       */
    if (STS.M) {                           /* 103046: BSKP ONE SSM                 */
        mode = STS.K ? -1 : 2;             /* 103052 SAA -1 (DirectOpen) / 103054 SAA 2 */
    } else {
        mode = STS.K ? 1 : 0;              /* 103060 SAA 1 (OldOpen) / 103062 A=0 (OpenFile) */
    }
 ...
 *   DOPEN prelude 103026 (M=1,K=1), shared entry 103041 (STD I 173, SAB 150),
