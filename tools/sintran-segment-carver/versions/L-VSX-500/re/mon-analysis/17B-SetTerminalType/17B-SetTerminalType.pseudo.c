/* ============================================================================
 * MON 17B  SetTerminalType (MSTTY)  -  pseudo-C model for an emulator
 * SINTRAN III VSX/500 L.
 *
 * Derived from the real disassembly (see 17B-SetTerminalType.ASM). MSTTY is the
 * SET entry into a large shared terminal-service module: a 2-word prefix that
 * presets a mode global, then FALLS THROUGH into MGTTY (MON 16B) - the SAME body.
 * Control flow and register moves are BYTE-VERIFIED; the field/global semantics
 * (which cell is the terminal type, the exact meaning of the mode global at
 * 000072 and the SSPTM bit) are INFERRED - the resident datafield and cell 000072
 * lie outside the carved window. Every instruction is translated per the canonical
 ...
 *   A = terminal type          (INPUT: appendix H type number)
 * Error return: A = error number. Public background users may only set the type
 * for their own terminal. */

int mon_setterminaltype(mon_regs *r)
{
    /* --- 044626 MSTTY SET prefix: preset the mode/opcode global to 20B --- */
    mem[000072] = 020;                    /* 044626/044627: SAA 20 / STA I 146    */
                                          /*   (mem[000072] reached via link cell 044775) */
    /* falls through into MGTTY = 044630 (the shared get/set body) */

    /* --- 044630 shared body: mark the set/get discriminator nonzero --- */
    ...
    /* 044634 JMP 3 -> 044637 (044635-044636 flag-clear prologue is the STMT1
     * sibling entry, not reached here) */
    ...
    /* --- 044661 set/get fork: the SET path (17B) TAKES the datafield WRITE --- */
    if (r->Bframe[0164] != 0) {           /* 044661/044662: LDA ,B 164 / JAZ 4    */
        df[027] |= 1;                     /* 044663-044665: LDA/BSET ONE 0/STA    */
                                          /*   write bit0 of terminal datafield[X+27] */
    }
    ...
    rdiv_ST(r);                           /* 044710 RDIV ST: A=(A:D)/T ; D=rem     */
 ...
 *   MSTTY SET prefix 044626-044627 (SAA 20 / STA I 146 -> mode global via cell 044775),
 *   fall-through into MGTTY 044630, set/set discriminator B+164 (044633/044661),
 *   the SET-only datafield write df[027] |= 1 (044663-044665),
 ...
 *
 * The MON 17B -> MSTTY link is NOT byte-proven: GOTAB[17] = 120526 vectors to the
 * F1616 stub (025-S3IRPIT), which transfers through the uncarved resident
 * CALLPROC bridge; the value 044626 appears nowhere the stub dereferences.
 * Attribution rests on the MSTTY symbol name + the fall-through into MGTTY. The
