...
- **Reply build `maybe_build_xrout_message @0xBFF8`:** assembles a descriptor array (each entry 12 bytes)
  then sends via multicall header `(0x8a)=0x24 fn`, `(0x90)=count`, `trap #2`. Descriptors seen:
  type 6 `{len, subcode, ptr}`, type 7 `{handle, 0, ptr}`, type 7 `{r6, 0xffff, ptr}`, type 0xc
  `{big, identity=low16(0x1E21A)}`.

Pseudocode (handle sources explicit):
```
on_enns0_request():
    ...
    status = XFMST(HANDLE)                            # HANDLE is INPUT
    desc = [ (6,len,subcode,ptr), (7,HANDLE,0,ptr), (7,r6,0xffff,ptr), (0xc, identity=low16(port)) ]
    XFSMC(HANDLE, desc)                               # send reply; HANDLE is INPUT
```
