...
- **ND-120 side:** the "N-100 Octobus/MPM Line Driver" card - two board versions
  exist: PCB 3109 (part 324118, OBCON gate array revision 36600B) and PCB 3096
  (part 324133, revision 36600D, square ASIC package). This is the card
  ND-05.020.01 calls the MFbus Line Driver: a Multiport Line Driver with an added
  ND-OBCON octobus controller gate array and differential line transceivers.
  Both boards: LEDs LD1 = transmit request (XREQ), LD2 = MASTER; a remove-master
  switch; thumbwheels for octobus SPEED, STATION number (low/high) and octobus
  DEVICE NUMBER; connectors A = address + control, B = data + control (the
  multiport line-driver channel cable pair to the MFbus/MPM cabinet), C =
  standard ND-100 system bus. Full catalogue detail in dossier 4.11.
  Small-cabinet variant: the Double Bus Controller (part 324244) combines MFbus
  Controller + Line Driver + Port on one card with dedicated Double Bus
  Backwiring.
...
| 2026-07-20 | Independent corroboration recorded in [NDIX-KERNEL-INTERFACE-EVIDENCE-2026-07-20.md](NDIX-KERNEL-INTERFACE-EVIDENCE-2026-07-20.md): NDIX kernel source (guest-OS side of MON 600B / PC_OMC callg, FE_INIT handshake), ND-05.012.01 §13 primary text for §5.4/§6.2/§6.4, NEC-01 unit-code tables showing 5015 registers are microcode unit selects (supports §10.2); C7 partial answer. Remaining §10.3-contaminated docs (60B_N500M_Hardware_Mapping, ND500-MONITOR-CALL-ARCHITECTURE body, both Emulator docs) corrected/deprecated |
