...
------  -----   -----------   ---------------------------------------------------
+0      b0-b1   0b 02         constant on every frame observed
+2      b2      20 | 3f       frame kind: 0x20 carries data, 0x3f is an ACK
...
+9..10  plen    0030          number of bytes following this header
```

**The length relation holds on every captured frame**, in both directions and for both frame
kinds:
...
so `plen` is the authoritative payload length and the 802.3 length is derivable from it. An ACK
carries `plen = 0` and the frame is then padded out to the 60-byte minimum.

**The link ids are per-node and swap with direction.** Node 102 is `0x5062` and node 100 is
`0x59c1` throughout the capture; a frame from 102 to 100 carries `id1=5062 id2=59c1`, and the
...
**UNKNOWN**: whether b2 takes values other than `0x20` and `0x3f` (no others were observed);
whether there is a negative acknowledgement / reject; the window size; the retransmit timer.
...
### What this requires of the C# implementation [STATUS: NOT IMPLEMENTED]

As of 2026-08-01 the C# node **cannot relay**. `SintranHeader.Marker2Relay` (`0x12`) is defined in
`Xmsg.Protocol\Wire\SintranHeader.cs` but is **never assigned**: every send path in the tree sets
`Marker2Normal`, and no code inspects whether a received datagram is addressed to somebody else.
