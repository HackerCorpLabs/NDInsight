...
A second transport was built today - COSMOS over Ethernet, plus a C# node (D19999) on a shared
segment with the live machines. That work touches three conclusions in this plan. None of the five
answered agenda items is overturned; two are **strengthened by new evidence** and one moves from
"worth doing" to **required**.
...
| --- | --- | --- |
| `LapbLayerAdapter` | HDLC/LAPB | no |
| `EthernetLink` | 802.3 + LLC1 + ND link layer | no |
...
table, a sequence store and a set of `IXmsgServer`s, and wires up the detector, codec, `XmsgLayer`
and `XmsgServerHost`. The HDLC runner and the Ethernet runner now share it.

That is the composition root 8.2 describes, already written and already proving the point it was
meant to prove: the ENTIRE node above the link is transport-neutral. It was extracted precisely so
the Ethernet path would not become a second copy, and the fact that it dropped straight onto a
completely different transport with no changes is the strongest evidence yet for the 8.1 boundary.
...
9.5 asked whether a host process serves one node number or several, and recommended "one, with N
links" - as a design preference, since the HDLC runner only ever had one link.
...
That is not a limitation that can be papered over by listening wider, and the reason is specific:
**each peer is a separate link with its own learned link id and its own datagram sequence.** Flags 1
is one sequence per direction per link. Two peers on one segment are two links in every sense that
matters, sharing only a backend.
...
**Scope limit, restated because it matters:** the rule was read off HDLC frames. A relayed frame has
never been captured on Ethernet. The topology to capture one now exists (D103 on HDLC to D100, the
segment on the other side), so this is answerable rather than assumed - but it is NOT yet answered.
...
Consequences for the split:
 ...
 - `Cosmos.Ethernet` is NOT feature-complete. A node that cannot answer `0x0F` cannot be addressed
   by a real ND machine, whatever else works.
 - The "peer link id is learned from the sender field" model in `NdLinkLayer` does not obviously hold
   for this frame: the sender field is 0 and the receiver field is a value we never advertised. Either
   `0x0F` uses those positions differently, or the field mapping is wrong for it. UNKNOWN either way -
   the bytes are being captured before anything is concluded.

This is the first genuinely new wire fact from the Ethernet work, and it belongs in the transport
package's scope, not the server's.

### 15.6 What is NOT claimed here

 - That `0x0F` is a connect, an open, or anything else. Its 100-byte payload has not been decoded.
 - That the Ethernet relay behaves like the HDLC relay. Untested, and 15.4 says so.
 - That `XmsgNodeHost` is finished. It has no multi-link support, which is exactly 15.3.
 - That the Ethernet path works end to end against a real machine. It does not yet: one frame arrived,
   was not answered, and nothing followed. The COSMOS Ethernet path on the live machines is itself
   currently misbehaving (HDLC works between all three nodes; Ethernet does not), and whether that is
   a machine-side configuration problem or something our node provokes is UNKNOWN.
