...
namespace NDInsight.Sintran.Xmsg.Api.Node
{
    ...
    public sealed class XmsgKernelServer : IXmsgServer, IXmsgDatagramSink
    {
        ...
        /// </returns>
        /// <remarks>
        /// The trailer starts after the 13-byte SINTRAN header and the 19-byte XMSG sub-header,
        /// and its length is the sub-header's XMLEN. Falling back to the whole remainder when
        /// XMLEN overruns keeps a malformed frame from throwing here, and falling back to
        ...
        private static byte[] ExtractTrailer(XmsgFrame frame)
        {
            ...
            int length = frame.SubHeader != null ? frame.SubHeader.UserDataLength : available;
